...
def inextensible_rope():
    ...
    model1.add(
        Spherical_joint2D(frame_left, inextensible_bernoulli1, r_OB1, frame_ID2=(0,))
    )
    model1.add(frame_right)
    model1.add(
        Linear_guidance_xyz_2D(
            frame_right,
            inextensible_bernoulli1,
            r_OB2,
            frame_right.A_IK(0),
            frame_ID2=(1,),
        )
    ...
    model1.add(
        Translational_f_pot(
            Linear_spring(k, l0), inextensible_bernoulli1, frame_right, frame_ID1=(1,)
        )
    )
    ...
    solver = Newton(
        model1, n_load_steps=20, max_iter=20, tol=1.0e-6, numerical_jacobian=False
    )
    ...
    inextensible_bernoulli2 = Inextensible_Euler_bernoulli(
        ...
        q0=sols[0].q[-1],
        u0=u0,
        la_g0=sols[0].la_g[-1, :-3],
    )

    model2.add(inextensible_bernoulli2)
    model2.add(frame_left)
    model2.add(
        Spherical_joint2D(frame_left, inextensible_bernoulli2, r_OB1, frame_ID2=(0,))
    )
    ...
    model2.add(
        Spherical_joint2D(
            frame_right, inextensible_bernoulli2, r_OB2(0), frame_ID2=(1,)
        )
    )
    ...
    solver = Newton(
        model2, n_load_steps=50, max_iter=50, tol=1.0e-6, numerical_jacobian=False
    )
    ...
    inextensible_bernoulli3 = Inextensible_Euler_bernoulli(
        ...
        q0=sols[1].q[-1],
        u0=u0,
        la_g0=sols[1].la_g[-1, :-4],
    )

    model3.add(inextensible_bernoulli3)
    model3.add(frame_left)
    model3.add(
        Spherical_joint2D(frame_left, inextensible_bernoulli3, r_OB1, frame_ID2=(0,))
    )
    ...
    model3.add(
        Spherical_joint2D(
            frame_right, inextensible_bernoulli3, r_OB2(0), frame_ID2=(1,)
        )
    )
    ...
    solver = Newton(
        model3, n_load_steps=50, max_iter=50, tol=1.0e-6, numerical_jacobian=False
    )
...
def cantilever():
    ...
    inextensible_bernoulli = Inextensible_Euler_bernoulli(
        A_rho0, material_model, p, nEl, nQP, Q=Q, q0=q0, u0=u0
    )
    ...
    if statics:
        ...
        # vtk export
        beams[0].post_processing(
            sols[0].t, sols[0].q, "CantileverA", u=sols[0].u, binary=True
        )
        beams[1].post_processing(
            sols[1].t, sols[1].q, "CantileverB", u=sols[1].u, binary=True
        )
