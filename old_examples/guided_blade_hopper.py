...
from cardillo.model.bilateral_constraints.explicit import (
    Revolute_joint as Revolute_joint_expl,
)
from cardillo.model.bilateral_constraints.implicit import (
    Revolute_joint2D as Revolute_joint_impl,
)
...
if __name__ == "__main__":
    ...
    hip = add_rotational_forcelaw(hip_force_law, Revolute_joint_expl)(
    ...
    ground = Sphere_to_plane2D(
        frame, blade, 0, mu, frame_ID=(0,), prox_r_N=r_N, prox_r_T=r_N, e_N=e_N, e_T=e_T
    )
    model.add(ground)
    ...
    solver = Generalized_alpha_3(
        model, t1, dt, rho_inf=0.5, newton_tol=1e-6, numerical_jacobian=0
    )
