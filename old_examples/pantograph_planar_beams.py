...
if __name__ == "__main__":
    ...
    r_OP_l = lambda t: np.array([0, H / 2, 0]) + np.array(
        [t * displacementX_l, t * displacementY_l, 0]
    )
    A_IK_l = lambda t: A_IK_basic_z(t * rotationZ_l)

    r_OP_r = lambda t: np.array([L, H / 2, 0]) + np.array(
        [t * displacementX_r, t * displacementY_r, 0]
    )
    ...
    for bcol in range(0, nCol, 2):
        for brow in range(0, nRow, 2):
            ...
            if E_B_beam:
                ...
                model.add(
                    Rigid_connection2D(
                        beam1, beam2, r_OB, frame_ID1=frame_ID1, frame_ID2=frame_ID2
                    )
                )
    ...
    for brow in range(0, nRow, 2):
        for bcol in range(0, nCol - 1):
            ...
            if E_B_beam:
                ...
                model.add(
                    add_rotational_forcelaw(spring, Revolute_joint2D)(
                        beam1,
                        beam2,
                        r_OB,
                        np.eye(3),
    ...
    ######################
    solver = Newton(
        model, n_load_steps=3, max_iter=50, tol=1.0e-2, numerical_jacobian=False
    )
    ...
    post_processing(
        beams, sol.t, sol.q, f"Pantographic_sheet_{nRow}x{nCol}_statics", binary=True
    )
    # if statics:
    #     fig, ax = plt.subplots()
    #     ax.set_xlabel('x [m]')
