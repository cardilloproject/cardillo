...
def init_guess(continuum, t, t_new, x):
    ...
    for a in range(continuum.mesh.nn):
        ...
        p_left = 1 - p_right
        z_new[np.array([a, a + continuum.mesh.nn])] = (
            z_new[np.array([a, a + continuum.mesh.nn])]
            + left_disp[a_eta] * p_left
            + right_disp[a_eta] * p_right
        )
    return z_new[continuum.fDOF]


def first_gradient_solve(
    case="test_a", n_load_steps=20, source="Treolar", element_shape=(45, 15)
):
    ...
    filepath = folderpath / (
        filename + ".dill"
    )  # os.path.join(folderpath, filename + ".dill")
    ...
    if source == "Treolar":
        ...
        mesh = Mesh2D(
            knot_vectors, QP_shape, derivative_order=1, basis="B-spline", nq_n=2
        )
    ...
    if source == "Treolar_lagrange":
        ...
        mesh = Mesh2D(
            knot_vectors, QP_shape, derivative_order=1, basis="lagrange", nq_n=2
        )
    ...
    solver = Newton(
        ...
        tol=tol,
        max_iter=max_iter,
        init_guess=init_guess_continuum,
    )
    sol = solver.solve()
...
def pantographic_sheet_solve(
    ...
    if source == "Maurin" or source == "MaurinSquare":
        continuum = Pantographic_lattice(
            None, mat_param, mesh, Z, z0=z0, cDOF=cDOF, b=b
        )
    elif source == "Barchiesi" or source == "Barchiesi_newFormat":
        continuum = Bipantographic_lattice(
            None, mat_param, mesh, Z, z0=z0, cDOF=cDOF, b=b
        )
    ...
    # test init_guess
    continuum.q0 = init_guess_continuum(
        0, load_steps[0], z0[continuum.fDOF]
    )  # assembler callback
...
def pantographic_sheet_post(
    filepath, project_to_reference=False, case=None, last_only=False
):
    ...
    if not last_only:
        continuum.post_processing(
            sol.t,
            sol.q,
            filepath.parent / filepath.stem,
            project_to_reference=project_to_reference,
        )
    else:
        continuum.post_processing(
            sol.t[:: len(sol.t) - 1],
...
def standard_displacements(mesh, Z, case, source="Maurin"):
    ...
    if case == "test_a":
        ...
        center = np.array(
            [
                np.mean([Z[cDOF2x[0]], Z[cDOF2x[-1]]]),
                np.mean([Z[cDOF2y[0]], Z[cDOF2y[-1]]]),
            ]
        )
        displacement = (
            R @ (Z[np.array([cDOF2x, cDOF2y])] - center[:, None])
            + center[:, None]
            - Z[np.array([cDOF2x, cDOF2y])]
            + translation[:, None]
        )
        ...
        displacement1 = (
            R1 @ (Z[np.array([cDOF1x, cDOF1y])] - center1[:, None])
            + center1[:, None]
            - Z[np.array([cDOF1x, cDOF1y])]
        )
        displacement2 = (
            R2 @ (Z[np.array([cDOF2x, cDOF2y])] - center2[:, None])
            + center2[:, None]
            - Z[np.array([cDOF2x, cDOF2y])]
            + translation[:, None]
        )
    ...
    if case == "simple_shear":  #
        ...
        # cDOF2 = np.hstack((cDOF2x, cDOF2y))
        cDOF = np.hstack(
            (cDOF1x, cDOF2x, cDOF3x, cDOF4x, cDOF1y, cDOF2y, cDOF3y, cDOF4y)
        )
        ...
        b = lambda t: np.hstack(
            (b1x(t), b2x(t), b3x(t), b4x(t), b1y(t), b2y(t), b3y(t), b4y(t))
        )
...
def task_queue_solve():
    ...
    first_gradient_solve(
        case="test_a",
        n_load_steps=20,
        source="Treolar_lagrange",
        element_shape=(30, 10),
    )
