...
class NonsmoothPartitionedHalfExplicitEuler:
    ...
    def g(self, t, q, u, z, update_index_set=True):
        ...
        c_gk1 = self.model.g(t, q)
        # c_gk1 = self.model.g_dot(t, q, u)
        c_gammak1 = self.model.gamma(t, q, u)
        ...
        # # regularized normal contact
        # alpha = 1 - 1.0e-3
        # g_N_bar = g_Nk1 + (1 - alpha) * self.dt * g_N_dotk1
        # c_Nk1 = g_N_bar - prox_R0_np(g_N_bar - prox_r_N * la_N)
        ...
        ##########
        mu = self.model.mu
        c_Fk1 = la_F.copy()  # this is the else case => P_Fk1 = 0
        for i_N, i_F in enumerate(self.model.NF_connectivity):
            ...
            if len(i_F) > 0:
                ...
                if self.Ak1[i_N]:
                    # c_Fk1[i_F] = -la_F[i_F] - prox_sphere(
                    #     -la_F[i_F] + prox_r_F[i_N] * xi_Fk1[i_F],
                    #     mu[i_N] * la_N[i_N],
                    # )
                    c_Fk1[i_F] = -la_F[i_F] - prox_sphere(
                        -la_F[i_F] + prox_r_F[i_N] * gamma_Fk1[i_F],
                        mu[i_N] * la_N[i_N],
                    )

        ck1 = np.concatenate((c_gk1, c_gammak1, c_Nk1, c_Fk1))
    ...
    def Ry(self, yk1):
        ...
        # quantities of the converged impact free motion
        tk1 = self.tk1
        qk1 = self.qk1
        ...
        # Ry[nu : nu + nla_g] = La_gk1
        # Ry[nu + nla_g : nu + nla_g + nla_gamma] = La_gammak1
        ...
        # tangent impact law
        ####################
        # Ry[nu + nla_g + nla_gamma + nla_N :] = La_Fk1
    ...
    def Jy(self, yk1):
        return csr_matrix(approx_fprime(yk1, self.Ry))
    ...
    def step(self):
        ...
        ##################################
        # simple partitioned forward Euler
        ##################################
        tk1 = tk + h
        ...
        zk1, converged, error, j = fsolve_(z0, g, g_z)
        ...
        # store state and Lagrange multipliers of the impact free motion
        self.tk1 = tk1
        self.qk1 = qk1
        self.uk1_free = uk1_free.copy()

        self.la_gk1 = la_gk1 = zk1[: self.nla_g]
        self.la_gammak1 = la_gammak1 = zk1[self.nla_g : self.nla_g + self.nla_gamma]
        self.la_Nk1 = la_Nk1 = zk1[
            self.nla_g + self.nla_gamma : self.nla_g + self.nla_gamma + self.nla_N
        ]
        self.la_Fk1 = la_Fk1 = zk1[self.nla_g + self.nla_gamma + self.nla_N :]
        ...
        res = fsolve_(sk1, self.Ry, self.Jy)
        sk1 = res[0]
        converged *= res[1]
        error += res[2]
        j += res[3]
    ...
    def solve(self):
        ...
        for _ in pbar:
            ...
            q.append(qk1)
            u.append(uk1)
            P_g.append(la_gk1 * self.dt + La_gk1)
            P_gamma.append(la_gammak1 * self.dt + La_gammak1)
            P_N.append(la_Nk1 * self.dt + La_Nk1)
            P_F.append(la_Fk1 * self.dt + La_Fk1)
...
# TODO:
# - Why is stabilization with g_NK1 destroying the solver?
# - Invstigate order of convergence
...
class NonsmoothHalfExplicitRungeKutta:
    ...
    def gen_c(self, ti, Yi_fun):
        def c(Z, update_index_set=True):
            ...
            c_Nk1_stab = np.minimum(g_Nk1, mu_N)
            # c_Nk1_stab = mu_N
            ...
            c_Nk1 = np.where(
                ...
                np.minimum(g_N_dotk1, P_N),
                # np.minimum(xi_Nk1, P_N),
                P_N,
            )
