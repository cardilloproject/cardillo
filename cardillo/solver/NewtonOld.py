...
from scipy.sparse.linalg import (
    splu,
)  # SuperLU direct solver/ umfpack if scikit-umfpack is installed
...
class NewtonOld:
    ...
    def _jacobian_an(self, t, x):
        ...
        return bmat([[K, self.W_g], [g_q, None]], format="csr")
    ...
    def __init__(
        ...
        tol=1e-8,
        max_iter=50,
        sparse_solver="scipyLU",
        iterative_tol=1.0e-10,
        ...
        # chose sparse solver
        sparse_direct_solvers = {
            "superLU": lambda A, b: splu(A).solve(b),
            "scipyLU": lambda A, b: spsolve(A, b, use_umfpack=False),
            "umfpack": lambda A, b: spsolve(A, b, use_umfpack=True),
        }
        sparse_iterative_solvers = {
            "cgs": lambda A, b, M: cgs(A, b, M=M, tol=iterative_tol),
    ...
    def solve(self):
        ...
        for i in pbar:
            ...
            self.model.pre_iteration_update(
                self.load_steps[i], self.x[i, : self.nq], self.u
            )
            ...
            pbar.set_description(
                f" force iter {i+1:>{len_t}d}/{self.nt}; Newton steps {k:>{len_maxIter}d}/{self.max_iter}; error {error:.4e}/{self.tol:.2e}"
            )
            ...
            if k > self.max_iter:
                ...
                pbar.close()
                print(
                    f"Newton-Raphson method not converged, returning solution up to iteration {i+1:>{len_t}d}/{self.nt}"
                )
                return Solution(
                    t=self.load_steps,
        ...
        pbar.close()
        return Solution(
            t=self.load_steps, q=self.x[:, : self.nq], la_g=self.x[:, self.nq :]
        )
