...
class Generalized_alpha_2:
    ...
    def __R_gen_analytic(self, tk1, xk1):
        ...
        La_Nk1 = xk1[
            3 * nu + nla_g + nla_gamma + nla_N : 3 * nu + nla_g + nla_gamma + 2 * nla_N
        ]
        la_Nk1 = xk1[
            3 * nu
            + nla_g
        ...
        R[3 * nu + nla_g + nla_gamma + 3 * nla_N + T_slip_ind] = P_T[T_slip_ind] + (
            (self.model.mu[N_slip_ind] * P_N[N_slip_ind] / norm_xi).reshape(-1, 1) * tmp
        ).reshape(-1)

        R[3 * nu + nla_g + nla_gamma + 3 * nla_N + nla_T + T_open_ind] = la_Tk1[
            T_open_ind
        ]
        R[
            3 * nu + nla_g + nla_gamma + 3 * nla_N + nla_T + T_stick_ind
        ] = gamma_T_dot_post[T_stick_ind]
        tmp = gamma_T_post[T_slip_ind_mat]
        norm_xi_ = np.linalg.norm(tmp, axis=-1)
        norm_xi = np.ones_like(norm_xi_)
        norm_xi[norm_xi_ > 0] = norm_xi_[norm_xi_ > 0]
        R[3 * nu + nla_g + nla_gamma + 3 * nla_N + nla_T + T_slip_ind] = la_Tk1[
            T_slip_ind
        ] + (
    ...
    def step(self):
        ...
        xk1[
            3 * nu + nla_g + nla_gamma : 3 * nu + nla_g + nla_gamma + nla_N
        ] = self.kappa_Nk
        xk1[
            3 * nu + nla_g + nla_gamma + nla_N : 3 * nu + nla_g + nla_gamma + 2 * nla_N
        ] = self.La_Nk
