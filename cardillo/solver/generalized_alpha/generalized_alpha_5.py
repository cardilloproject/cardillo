...
class Generalized_alpha_5:
    ...
    def __R_gen_analytic(self, tk1, xk1):
        ...
        kappa_ck1 = xk1[
            3 * nu + 3 * nla_g + nla_gamma : 3 * nu + 3 * nla_g + nla_gamma + nka_c
        ]
        ...
        R_x = bmat(
            [
                [Ra_a, Ra_U, Ra_Q, None, None, -W_gk1, -W_gammak1, Ra_ka_c],
...
class Generalized_alpha_5__:
    def __init__(
        ...
        self.nR_smooth = (
            3 * self.nu + self.nq + 3 * self.nla_g + self.nla_gamma + self.nka_c
        )
    ...
    def __R_gen_analytic(self, tk1, xk1):
        ...
        R[3 * nu + nq + nla_g : 3 * nu + nq + 2 * nla_g] = self.model.g_dot(
            tk1, qk1, uk1
        )
        R[3 * nu + nq + 2 * nla_g : 3 * nu + nq + 3 * nla_g] = self.model.g_ddot(
            tk1, qk1, uk1, ak1
        )
        R[
            3 * nu + nq + 3 * nla_g : 3 * nu + nq + 3 * nla_g + nla_gamma
