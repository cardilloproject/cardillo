...
class Generalized_alpha_5():
    ...
    def __init__(self, model, t1, dt, \
        ...
        self.nka_c = model.nka_c

        self.nR = 3 * self.nu + 3 * self.nla_g + self.nla_gamma + self.nka_c
        ...
        self.kappa_ck = model.ka_c0
        self.ak = spsolve(model.M(t0, model.q0).tocsr(), self.model.h(t0, model.q0, model.u0) + self.model.W_g(t0, model.q0) @ model.la_g0 + self.model.W_gamma(t0, model.q0) @ model.la_gamma0)
    ...
    def __R_gen_analytic(self, tk1, xk1):
        ...
        kappa_ck1 = xk1[3*nu+3*nla_g+nla_gamma:3*nu+3*nla_g+nla_gamma+nka_c]
        ...
        qk1 = self.qk + dt * self.model.q_dot(self.tk, self.qk, self.uk) + dt2 * self.model.q_ddot(self.tk, self.qk, self.uk, a_beta) + self.model.B(self.tk, self.qk) @ Qk1 + kappa_ck1 @ self.c_qk
        ...
        R[3*nu+3*nla_g+nla_gamma:3*nu+3*nla_g+nla_gamma+nka_c] = self.model.c(tk1, qk1)
        yield R
        
        # R[:nu] = Mk1 @ ak1 - ( self.model.h(tk1, qk1, uk1) + W_gk1 @ la_gk1 + W_gammak1 @ la_gammak1 + W_Nk1 @ la_Nk1 + W_Tk1 @ la_Tk1)
        Ra_q = self.model.Mu_q(tk1, qk1, ak1) - (self.model.h_q(tk1, qk1, uk1) + self.model.Wla_g_q(tk1, qk1, la_gk1) + self.model.Wla_gamma_q(tk1, qk1, la_gammak1))
        Ra_u = -self.model.h_u(tk1, qk1, uk1)
        Ra_a = Mk1 + Ra_q @ self.q_a + Ra_u * self.u_a
        Ra_U = Ra_u
        Ra_Q = Ra_q @ self.q_Q
        Ra_ka_c = Ra_q @ self.c_qk.T
        
        # R[nu:2*nu] = Mk1 @ Uk1 - W_Nk1 @ La_Nk1 - W_Tk1 @ La_Tk1
        RU_q = self.model.Mu_q(tk1, qk1, Uk1) - self.model.Wla_g_q(tk1, qk1, La_gk1)
        RU_a = RU_q @ self.q_a
        RU_Q = RU_q @ self.q_Q
        RU_ka_c = RU_q @ self.c_qk.T
        
        # R[2*nu:3*nu] = Mk1 @ Qk1 - W_Nk1 @ kappa_Nk1
        RQ_q = self.model.Mu_q(tk1, qk1, Qk1) - self.model.Wla_g_q(tk1, qk1, kappa_gk1)
        RQ_a = RQ_q @ self.q_a
        RQ_Q = Mk1 + RQ_q @ self.q_Q
        RQ_ka_c = RQ_q @ self.c_qk.T

        # R[3*nu:3*nu+nla_g] = self.model.g(tk1, qk1)
        Rka_g_q = self.model.g_q(tk1, qk1)
        Rka_g_a = Rka_g_q @ self.q_a
        Rka_g_Q = Rka_g_q @ self.q_Q
        Rka_g_ka_c = Rka_g_q @ self.c_qk.T
        ...
        RLa_g_ka_c = RLa_g_q @ self.c_qk.T
        ...
        Rla_gamma_ka_c = Rla_gamma_q @ self.c_qk.T

        # R[3*nu+3*nla_g+nla_gamma:3*nu+3*nla_g+nla_gamma+nka_c] = self.model.c(tk1, qk1)
        Rka_c_q = self.model.c_q(tk1, qk1)
        Rka_c_a = Rka_c_q @ self.q_a
        Rka_c_Q = Rka_c_q @ self.q_Q
        Rka_c_ka_c = Rka_c_q @ self.c_qk.T
        ...
        R_x =  bmat([[Ra_a, Ra_U, Ra_Q, None, None,  -W_gk1,  -W_gammak1, Ra_ka_c], \
                     [RU_a, Mk1,  RU_Q, None, -W_gk1,  None,        None, RU_ka_c], \
                     [RQ_a,  None, RQ_Q,  -W_gk1, None, None,        None, RQ_ka_c], \
                     [Rka_g_a,  None, Rka_g_Q,   None, None,None,    None, Rka_g_ka_c], \
                     [RLa_g_a,  RLa_g_u, RLa_g_Q,  None, None, None, None, RLa_g_ka_c], \
                     [Rla_g_a,  Rla_g_u, Rla_g_Q,   None, None,None, None, Rla_g_ka_c], \
                     [Rla_gamma_a, Rla_gamma_u, Rla_gamma_Q, None, None, None, None, Rla_gamma_ka_c], \
                     [Rka_c_a, None, Rka_c_Q, None, None, None, None, Rka_c_ka_c] \
                    ], format='csc')
    ...
    def step(self):
        ...
        xk1[3*nu+3*nla_g+nla_gamma:3*nu+3*nla_g+nla_gamma+nka_c] = self.kappa_ck
        ...
        return (converged, j, error), tk1, ak1, Uk1, Qk1, kappa_gk1, La_gk1, la_gk1, la_gammak1, kappa_ck1
    ...
    def solve(self): 
        ...
        kappa_c = [self.kappa_ck]

        pbar = tqdm(np.arange(self.t0, self.t1, self.dt))
        for _ in pbar:
            Bk = self.model.B(self.tk, self.qk)
            self.c_qk = self.model.c_q(self.tk, self.qk)
            self.q_a = dt2 * self.beta * self.alpha_ratio * Bk
            self.q_Q = Bk
            self.u_a = dt * self.gamma * self.alpha_ratio

            (converged, n_iter, error), tk1, ak1, Uk1, Qk1, kappa_gk1, La_gk1, la_gk1, la_gammak1, kappa_ck1 = self.step()
            ...
            self.kappa_ck = kappa_ck1
            self.a_bark = a_bark1

        # write solution
        return Solution(t=np.array(t), q=np.array(q), u=np.array(u), a=np.array(a), kappa_g=np.array(kappa_g), La_g=np.array(La_g), la_g=np.array(la_g), la_gamma=np.array(la_gamma), kappa_c=np.array(kappa_c))

class Generalized_alpha_5__():
    def __init__(self, model, t1, dt, \
                    rho_inf=1, beta=None, gamma=None, alpha_m=None, alpha_f=None,\
                       newton_tol=1e-8, newton_max_iter=40, newton_error_function=lambda x: np.max(np.abs(x)),\
                           numerical_jacobian=False, debug=False):
        ...
        self.nR_smooth = 3 * self.nu + self.nq + 3 * self.nla_g + self.nla_gamma + self.nka_c
    ...
    def __R_gen_analytic(self, tk1, xk1):
        ...
        Uk1 = xk1[nu:2*nu]
        Qk1 = xk1[2*nu:3*nu]
        Ck1 = xk1[3*nu:3*nu+nq]
        kappa_gk1 = xk1[3*nu+nq:3*nu+nq+nla_g]
        La_gk1 = xk1[3*nu+nq+nla_g:3*nu+nq+2*nla_g]
        la_gk1 = xk1[3*nu+nq+2*nla_g:3*nu+nq+3*nla_g]
        la_gammak1 = xk1[3*nu+nq+3*nla_g:3*nu+nq+3*nla_g+nla_gamma]
        kappa_ck1 = xk1[3*nu+nq+3*nla_g+nla_gamma:3*nu+nq+3*nla_g+nla_gamma+nka_c]
        ...
        uk1 = self.uk + dt * ((1-self.gamma) * self.a_bark + self.gamma * a_bark1) + Uk1
        a_beta = (0.5 - self.beta) * self.a_bark + self.beta * a_bark1 
        qk1 = self.qk + dt * self.model.q_dot(self.tk, self.qk, self.uk) + dt2 * self.model.q_ddot(self.tk, self.qk, self.uk, a_beta) + self.model.B(self.tk, self.qk) @ Qk1 + Ck1
        ...
        R[3*nu:3*nu+nq] = Ck1 - kappa_ck1 @ self.model.c_q(tk1, qk1)
        R[3*nu+nq:3*nu+nq+nla_g] = self.model.g(tk1, qk1)
        R[3*nu+nq+nla_g:3*nu+nq+2*nla_g] = self.model.g_dot(tk1, qk1, uk1)
        R[3*nu+nq+2*nla_g:3*nu+nq+3*nla_g] = self.model.g_ddot(tk1, qk1, uk1, ak1)
        R[3*nu+nq+3*nla_g:3*nu+nq+3*nla_g+nla_gamma] = self.model.gamma(tk1, qk1, uk1)
        R[3*nu+nq+3*nla_g+nla_gamma:3*nu+nq+3*nla_g+nla_gamma+nka_c] = self.model.c(tk1, qk1)
    ...
    def step(self):
        ...
        xk1[nu:2*nu] = self.Uk
        xk1[2*nu:3*nu] = self.Qk
        xk1[3*nu:3*nu+nq] = self.Ck
        xk1[3*nu+nq:3*nu+nq+nla_g] = self.kappa_gk
        xk1[3*nu+nq+nla_g:3*nu+nq+2*nla_g] = self.La_gk
        xk1[3*nu+nq+2*nla_g:3*nu+nq+3*nla_g] = self.la_gk
        xk1[3*nu+nq+3*nla_g:3*nu+nq+3*nla_g+nla_gamma] = self.la_gammak
        xk1[3*nu+nq+3*nla_g+nla_gamma:3*nu+nq+3*nla_g+nla_gamma+nka_c] = self.kappa_ck
        ...
        return (converged, j, error), tk1, ak1, Uk1, Qk1, Ck1, kappa_gk1, La_gk1, la_gk1, la_gammak1, kappa_ck1, kappa_Nk1, La_Nk1, la_Nk1, La_Tk1, la_Tk1
    ...
    def solve(self): 
        ...
        for _ in pbar:
            ...
            (converged, n_iter, error), tk1, ak1, Uk1, Qk1, Ck1, kappa_gk1, La_gk1, la_gk1, la_gammak1, kappa_ck1, kappa_Nk1, La_Nk1, la_Nk1, La_Tk1, la_Tk1 = self.step()
            pbar.set_description(f't: {tk1:0.2e}s < {self.t1:0.2e}s; Newton: {n_iter}/{self.newton_max_iter} iterations; error: {error:0.2e}')
            ...
            uk1 = self.uk + dt * ((1-self.gamma) * self.a_bark + self.gamma * a_bark1) + Uk1
            a_beta = (0.5 - self.beta) * self.a_bark + self.beta * a_bark1
            qk1 = self.qk + dt * self.model.q_dot(self.tk, self.qk, self.uk) + dt2 * self.model.q_ddot(self.tk, self.qk, self.uk, a_beta) + Bk @ Qk1 + Ck1
        ...
        # write solution
        return Solution(t=np.array(t), q=np.array(q), u=np.array(u), a=np.array(a), kappa_g=np.array(kappa_g), La_g=np.array(La_g), la_g=np.array(la_g), la_gamma=np.array(la_gamma), kappa_c=np.array(kappa_c), kappa_P=np.array(kappa_N), La_N=np.array(La_N), la_N=np.array(la_N), La_T=np.array(La_T), la_T=np.array(la_T), P_N=np.array(P_N), P_T=np.array(P_T))