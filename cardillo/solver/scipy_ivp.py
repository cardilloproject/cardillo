...
class ScipyIVP:
    ...
    def v(self, t, u):
        v = np.zeros(self.nv, dtype=float)
        v[self.fDOF_u] = u
    ...
    def eqm(self, t, x):
        ...
        # Note: We are not allowed to perform a copy here. Otherwise the
        # projection step is not applied to the final result!
        q = x[: self.nq]
        z = self.z(t, q)
        u = x[self.nq :]
        v = self.v(t, u)

        # Note: The projected values have to be written onto x again,
        # otherwise they are not applied to the final result!
        z, v = self.system.step_callback(t, z, v)
        x[: self.nq] = z[self.fDOF_q]
        x[self.nq :] = v[self.fDOF_u]
        ...
        v_dot = spsolve(A, np.concatenate([h, -zeta_g, -zeta_gamma]))[: self.nv]

        # # TODO: Can we use a sparse ldl decomposition here as done in C++?
        # # Dense version can be performed using scipy,
        # # see https://en.wikipedia.org/wiki/Cholesky_decomposition#Applications.
        # A = bmat([[        M, -W_g, -W_gamma], \
        #           [    W_g.T, None,     None], \
        #           [W_gamma.T, None,     None]], format="coo").toarray()
        # from scipy.linalg import ldl, solve_triangular
        # lu, d, perm = ldl(A, overwrite_a=True)
        # b = np.concatenate([h, -zeta_g, -zeta_gamma])
        # y = solve_triangular(lu, b, lower=False)
        # v_dot = solve_triangular(d @ lu.T, y, lower=True)[: self.nv]

        dx = np.zeros(self.nx, dtype=float)
        dx[: self.nq] = self.system.q_dot(t, z, v)[self.fDOF_q]
        dx[self.nq :] = v_dot[self.fDOF_u]
        return dx
