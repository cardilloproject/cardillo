...
class NewtonStepSizeControl:
    ...
    def __init__(
        ...
        raise RuntimeError("This is not ready for usage yet.")
    ...
    def __pbar_text_old(self, force_iter, newton_iter, error):
        return (
            f" force iter {force_iter+1:>{self.len_t}d}/{self.nt};"
            f" Newton steps {newton_iter+1:>{self.len_maxIter}d}/{self.max_iter};"
            f" error {error:.4e}/{self.atol:.2e}"
    ...
    def __pbar_text(self, t, newton_iter, error):
        return (
            f" t: {t:0.2e}s < {1.0:0.2e};"
            f" Newton steps {newton_iter+1:>{self.len_maxIter}d}/{self.max_iter};"
            f" error {error:.4e}/{self.atol:.2e}"
        )
    ...
    def solve(self):
        ...
        while t <= 1.0:
            ...
            if self.verbose:
                ...
                i1 = int(t // frac)
                pbar.update(i1 - i)
                pbar.set_description(self.__pbar_text(t, k, error))
                i = i1
            ...
            while (not converged) and (k < self.max_iter):
                ...
                # condA = condition_number(dR)
                condition_dR = np.linalg.cond(dR.toarray())
                print(f"condition_dR: {condition_dR:.3e}")

                # solve linear system of equations and perform update
                # x -= spsolve(dR, R)
                x -= spsolve(dR, R, use_umfpack=False)
            ...
            # update h such that k becomes target_iter
            h *= targert_iter / k
            ...
            if not converged:
                ...
                return Solution(
                    t=t_list,
                    q=x_list[: j + 1, : self.nq],
                    la_g=x_list[: j + 1, self.nq :],
                    la_N=x_list[: j + 1, self.nq + self.nla_g :],
                )

            # solver step callback
            x[: self.nq], _ = self.system.step_callback(t, x[: self.nq], self.u0)
    ...
    def solve_old(self):
        ...
        for i in pbar:
            ...
            # def fun(x):
            #     generator = self.__eval__(self.load_steps[i], x)
            #     R = next(generator)
            #     return 0.5 * R @ R
            # def jac(x):
            #     generator = self.__eval__(self.load_steps[i], x)
            ...
            while (not converged) and (k < self.max_iter):
                ...
                # solve linear system of equations and perform update
                # self.x[i] -= spsolve(dR, R)
                self.x[i] -= spsolve(dR, R, use_umfpack=False)
                # import sparseqr
                # self.x[i] -= sparseqr.solve(dR, R, tolerance=0)
                # from scipy.sparse.linalg import lsqr, lsmr
                # self.x[i] -= lsqr(dR, R, atol=0, btol=0, conlim=0)[0]
                # # self.x[i] -= lsmr(dR, R, atol=0, btol=0, conlim=0)[0]
                # from cardillo.math import qr_solve, qr_underdetermined_solve
                # # self.x[i] -= qr_solve(dR, R)
                # self.x[i] -= qr_underdetermined_solve(dR, R)
