...
from cardillo.beams._base import TimoshenkoPetrovGalerkinBase, RodExportBase


class DirectorAxisAngle(TimoshenkoPetrovGalerkinBase):
    # class DirectorAxisAngle(RodExportBase):
    if True:
    ...
    if False:
        ...
        def initial_configuration(
            ...
            for i in range(nn_r):
                v_C0[:, i] = v_P0 + cross3(
                    A_IK0 @ K_omega_IK0, (r_OC0[:, i] - r_OC0[:, 0])
                )
    ...
    if False:
        ...
        def f_pot_el_q(self, qe, el):
            ...
            for i in range(self.nquadrature):
                ...
                K_Gamma = K_Gamma_bar / J
                K_Gamma_bar_qe = (
                    np.einsum("k,kij", r_OP_xi, A_IK_qe) + A_IK.T @ r_OP_xi_qe
                )
    ...
    if False:
        ...
        def plot_frames(self, ax, q, n=10, length=1):
            r, d1, d2, d3 = self.frames(q, num=n)
