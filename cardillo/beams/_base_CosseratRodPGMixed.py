...
# class CosseratRodPGMixedByDomenico(RodExportBase, ABC):
#     def __init__(
#         self,
#         cross_section,
#         material_model,
...
#         # self.polynomial_degree_stress = polynomial_degree_stress 
...
#         # self.nquadrature_stress_dyn = nquadrature_stress_dyn
#         # self.nquadrature_stress = nquadrature_stress
#         # print(f"nquadrature_stress_dyn: {nquadrature_stress_dyn}")
#         # print(f"nquadrature_stress: {nquadrature_stress}")

#         # chose basis functions
#         self.basis_r = basis_r
#         self.basis_psi = basis_psi
#         # self.basis_stress = basis_stress
...
#         # if basis_stress == "Lagrange_Disc":
#         #     self.knot_vector_stress = LagrangeKnotVector(polynomial_degree_stress, nelement)
#         # else:
#         #     raise RuntimeError(f'wrong basis_stress: "{basis_stress}" was chosen')
...
#         # self.mesh_stress = Mesh1D(
#         #     self.knot_vector_stress,
#         #     nquadrature_stress,
#         #     dim_q=6, 
#         #     derivative_order=1,
#         #     basis=basis_stress,
...
#         self.nnodes_r = self.mesh_r.nnodes
#         self.nnodes_psi = self.mesh_psi.nnodes
#         self.nnodes_stress = self.mesh_stress.nnodes
...
#         self.nnodes_element_r = self.mesh_r.nnodes_per_element
#         self.nnodes_element_psi = self.mesh_psi.nnodes_per_element
#         self.nnodes_element_stress = self.mesh_stress.nnodes_per_element

#         # total number of generalized coordinates and velocities for each field
#         self.nq_r = self.mesh_r.nq
#         self.nq_psi = self.mesh_psi.nq
#         self.nq_stress = self.mesh_stress.nq
#         self.nu_r = self.mesh_r.nu
#         self.nu_psi = self.mesh_psi.nu
#         self.nu_stress = self.mesh_stress.nu

#         # total number of generalized coordinates and velocities
#         self.nq = self.nq_r + self.nq_psi + self.nq_stress
#         self.nu = self.nu_r + self.nu_psi + self.nu_stress

#         # number of generalized coordinates and velocities per element
#         self.nq_element_r = self.mesh_r.nq_per_element  
#         self.nq_element_psi = self.mesh_psi.nq_per_element 
#         self.nq_element_stress = self.mesh_stress.nq_per_element
#         self.nq_element = self.nq_element_r + self.nq_element_psi + self.nq_element_stress

#         self.nu_element_r = self.mesh_r.nu_per_element
#         self.nu_element_psi = self.mesh_psi.nu_per_element
#         self.nu_element_stress = self.mesh_stress.nu_per_element
#         self.nu_element = self.nu_element_r + self.nu_element_psi + self.nu_element_stress


#         # global element connectivity
#         self.elDOF_r = self.mesh_r.elDOF
#         self.elDOF_psi = self.mesh_psi.elDOF + self.nq_r
#         self.elDOF_stress = self.mesh_stress.elDOF + self.nq_psi + self.nq_r
#         self.elDOF_r_u = self.mesh_r.elDOF_u
#         self.elDOF_psi_u = self.mesh_psi.elDOF_u + self.nu_r
#         self.elDOF_stress_u = self.mesh_stress.elDOF_u + self.nu_psi + self.nu_r
#         # qe = q[elDOF[e]] "q^e = C_e,q q"

#         # global nodal
#         self.nodalDOF_r = self.mesh_r.nodalDOF
#         self.nodalDOF_psi = self.mesh_psi.nodalDOF + self.nq_r
#         self.nodalDOF_stress = self.mesh_stress.nodalDOF + self.nq_psi + self.nq_r
#         self.nodalDOF_r_u = self.mesh_r.nodalDOF_u
#         self.nodalDOF_psi_u = self.mesh_psi.nodalDOF_u + self.nu_r
#         self.nodalDOF_stress_u = self.mesh_stress.nodalDOF_u + self.nu_psi + self.nu_r

#         # nodal connectivity on element level, gives the nodalDOF expressed for each element
#         self.nodalDOF_element_r = self.mesh_r.nodalDOF_element
#         self.nodalDOF_element_psi = self.mesh_psi.nodalDOF_element + self.nq_element_r
#         self.nodalDOF_element_stress = self.mesh_stress.nodalDOF_element + self.nq_element_psi + self.nq_element_r
#         self.nodalDOF_element_r_u = self.mesh_r.nodalDOF_element_u
#         self.nodalDOF_element_psi_u = self.mesh_psi.nodalDOF_element_u + self.nu_element_r
#         self.nodalDOF_element_stress_u = self.mesh_stress.nodalDOF_element_u + self.nu_element_psi + self.nu_element_r
#         # r_OP_i^e = C_r,i^e * C_e,q q = C_r,i^e * q^e
#         # r_OPi = qe[nodelDOF_element_r[i]]
...
#         for el in range(nelement):
#             self.elDOF[el, : self.nq_element_r] = self.elDOF_r[el]
#             self.elDOF[el, self.nq_element_r : self.nq_element_stress] = self.elDOF_psi[el]
#             self.elDOF[el, self.nq_element_stress :] = self.elDOF_stress[el]

#             self.elDOF_u[el, : self.nu_element_r] = self.elDOF_r_u[el]
#             self.elDOF_u[el, self.nu_element_r : self.nu_element_stress] = self.elDOF_psi_u[el]
#             self.elDOF_u[el, self.nu_element_stress :] = self.elDOF_stress_u[el]
...
#         self.N_psi = self.mesh_psi.N
#         self.N_psi_xi = self.mesh_psi.N_xi
#         self.N_stress = self.mesh_stress.N
#         self.N_stress_xi = self.mesh_stress.N_xi

#         self.N_r_dyn = self.mesh_r_dyn.N
#         self.N_psi_dyn = self.mesh_psi_dyn.N
#         self.N_stress_dyn = self.mesh_stress_dyn.N
...
#         self.qp_stress = self.mesh_stress.qp  # quadrature points for the stress field
#         self.qw_stress = self.mesh_stress.wp  # quadrature weights for the stress field
#         self.qp_stress_dyn = self.mesh_stress_dyn.qp  
#         self.qw_stress_dyn = self.mesh_stress_dyn.wp  
...
#         self.q0 = Q.copy() if q0 is None else q0
#         self.u0 = np.zeros(self.nu, dtype=float) if u0 is None else u0
#         # copy() if q0 is None else q0: Questa riga assegna un valore all'attributo self.q0 dell'istanza della classe.
#         # Se l'argomento q0 non è stato fornito (ossia è None), allora viene creato un nuovo oggetto array copiando 
#         # il contenuto di Q, e questo nuovo array viene assegnato all'attributo self.q0. In caso contrario, se q0 è stato 
#         # fornito come argomento, viene assegnato direttamente a self.q0.

#         # evaluate shape functions at specific xi
#         self.basis_functions_r = self.mesh_r.eval_basis
#         self.basis_functions_psi = self.mesh_psi.eval_basis
#         self.basis_functions_stress = self.mesh_stress.eval_basis
...
#     def set_initial_stresses(self, Q):
#         self.Q_stress = Q.copy() # reference configuration
#         self.K_Stresses0 = np.zeros((self.nelement, self.nquadrature, 6), dtype=float)
        
#         for el in range(self.nelement):
#             qe_stress = self.Q_stress[self.elDOF[el]]

#             for i in range(self.nquadrature_stress):
#                 # current quadrature point
#                 qpi_stress = self.qp_stress[el, i]
#                 # evaluate required quantities
#                 K_Stresses = self._eval_stress(qe_stress, qpi_stress)
#                 # safe precomputed quantities for later
#                 self.K_Stresses0[el, i] = K_Stresses
#                 ''' definire la funzione _eval_stress'''


#             for i in range(self.nquadrature_stress_dyn):
#                 # current quadrature point
#                 qpi_stress = self.qp_stress_dyn[el, i]
#                 # evaluate required quantities
#                 K_Stresses = self._eval_stress(qe_stress, qpi_stress)
...
#     @staticmethod
#     def straight_configuration_stress(
#         polynomial_degree_r,
#         polynomial_degree_psi,
...
#         r_OP=np.zeros(3, dtype=float),
#         A_IK=np.eye(3, dtype=float),
#         vector_stress=np.zeros(6, dtype=float),
#         rotation_parameterization=QuaternionRotationParameterization(),
#     ):
#         if basis_r == "Lagrange":
#             nnodes_r = polynomial_degree_r * nelement + 1
...
#         if basis_stress == "Lagrange_Disc":
#             nnodes_stress = polynomial_degree_stress * nelement + 1
#         else:
#             raise RuntimeError(f'wrong basis_stress: "{basis_stress}" was chosen')
...
#         if basis_stress == "Lagrange_Disc":
#             nnodes_stress_1 = nelement - 1
#             dim = len(vector_stress)
#             hh = np.zeros((dim, nnodes_stress + nnodes_stress_1))
            
#         q_stress = hh.reshape(-1, order="C")
...
#         return np.concatenate([q_r, q_psi, q_stress])
...
#     def f_pot_el_mixed(self, qe, el):
#         f_pot_el = np.zeros(self.nu_element, dtype=qe.dtype)

#         for i in range(self.nquadrature_stress):
#             # extract reference state variables
#             qpi = self.qp_stress[el, i]
#             qwi = self.qw_stress[el, i]
#             J = self.J[el, i]
#             K_Gamma0 = self.K_Gamma0[el, i]
#             K_Kappa0 = self.K_Kappa0[el, i]
...
#             # evaluate stress quantities
#             N, T1, T2, M1, M2, M3 = self._eval_stress(qe, qpi)
#             ''' definire la funzione eval_stress for the CosseratRodPG_SE3_Mixed'''
...
#             K_n = self.material_model.K_n(K_Gamma, K_Gamma0, K_Kappa, K_Kappa0)
#             K_m = self.material_model.K_m(K_Gamma, K_Gamma0, K_Kappa, K_Kappa0)
#             CC = self.material_model.C(self.material_model.Ei,self.material_model.Fi)
...
#                 f_pot_el[self.nodalDOF_element_stress_u[node][0]] += (
#                     self.N_stress_xi[el, i, node] * (K_Gamma[0] - N/CC[0,0]) * J * qwi
#                 ) # axial stress

#                 f_pot_el[self.nodalDOF_element_stress_u[node][1]] += (
#                     self.N_stress_xi[el, i, node] * (K_Gamma[1] - T1/CC[1,1]) * J * qwi
#                 ) # shear 1

#                 f_pot_el[self.nodalDOF_element_stress_u[node][2]] += (
#                     self.N_stress_xi[el, i, node] * (K_Gamma[2] - T2/CC[2,2]) * J * qwi
#                 ) # shear 2

#                 f_pot_el[self.nodalDOF_element_stress_u[node][3]] += (
#                     self.N_stress_xi[el, i, node] * (K_Kappa[0] - M1/CC[3,3]) * J * qwi
#                 ) # torsion

#                 f_pot_el[self.nodalDOF_element_stress_u[node][4]] += (
#                     self.N_stress_xi[el, i, node] * (K_Gamma[4] - M2/CC[4,4]) * J * qwi
#                 ) # bending moment 1

#                 f_pot_el[self.nodalDOF_element_stress_u[node][5]] += (
#                     self.N_stress_xi[el, i, node] * (K_Gamma[5] - M3/CC[5,5]) * J * qwi
#                 ) # bending moment 2
