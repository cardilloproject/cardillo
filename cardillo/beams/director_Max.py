...
class Timoshenko_beam_director(metaclass=ABCMeta):
    def __init__(
        ...
        self.nn_el_di = nn_el_di = (
            polynomial_degree_di + 1
        )  # number of nodes per element

        # distinguish centerline and director meshes
        self.nq_n_r = (
            nq_n_r
        ) = 3  # number of degrees of freedom per node of the centerline
        self.nq_n_di = nq_n_di = (
            3 * 3
        )  # number of degrees of freedom per node of the director 1, 2 and 3

        self.mesh_r = Mesh1D(
            self.knot_vector_r, nQP, derivative_order=1, basis=basis, nq_n=nq_n_r
        )
        self.mesh_di = Mesh1D(
            self.knot_vector_di, nQP, derivative_order=1, basis=basis, nq_n=nq_n_di
        )
        ...
        self.nu = self.nq
        self.nq_el = (
            nn_el_r * nq_n_r + nn_el_di * nq_n_di
        )  # total number of generalized coordinates per element
        self.nq_el_r = nn_el_r * nq_n_r
        self.nq_el_di = (
            nn_el_di * 3
        )  # TODO: can we do this more verbose instead of hard coded 3?
    ...
    def f_pot_q_el(self, qe, el):
        ...
        for i in range(self.nQP):
            ...
            # compute contact forces and couples from partial derivatives of the strain energy function w.r.t. strain measures
            n1, n2, n3 = self.material_model.n_i(Gamma_i, Gamma0_i, Kappa_i, Kappa0_i)
            n_i_Gamma_i_j = self.material_model.n_i_Gamma_i_j(
                Gamma_i, Gamma0_i, Kappa_i, Kappa0_i
            )
            n_i_Kappa_i_j = self.material_model.n_i_Kappa_i_j(
                Gamma_i, Gamma0_i, Kappa_i, Kappa0_i
            )
            n1_qr, n2_qr, n3_qr = n_i_Gamma_i_j @ Gamma_j_qr

            n1_qd1, n2_qd1, n3_qd1 = (
                np.outer(n_i_Gamma_i_j[0], Gamma_1_qd1) + n_i_Kappa_i_j @ kappa_j_qe_d1
            )
            n1_qd2, n2_qd2, n3_qd2 = (
                np.outer(n_i_Gamma_i_j[1], Gamma_2_qd2) + n_i_Kappa_i_j @ kappa_j_qe_d2
            )
            n1_qd3, n2_qd3, n3_qd3 = (
                np.outer(n_i_Gamma_i_j[2], Gamma_3_qd3) + n_i_Kappa_i_j @ kappa_j_qe_d3
            )

            m1, m2, m3 = self.material_model.m_i(Gamma_i, Gamma0_i, Kappa_i, Kappa0_i)
            m_i_Gamma_i_j = self.material_model.m_i_Gamma_i_j(
                Gamma_i, Gamma0_i, Kappa_i, Kappa0_i
            )
            m_i_Kappa_i_j = self.material_model.m_i_Kappa_i_j(
                Gamma_i, Gamma0_i, Kappa_i, Kappa0_i
            )

            m1_qr, m2_qr, m3_qr = m_i_Gamma_i_j @ Gamma_j_qr
            m1_qd1, m2_qd1, m3_qd1 = (
                np.outer(m_i_Gamma_i_j[0], Gamma_1_qd1) + m_i_Kappa_i_j @ kappa_j_qe_d1
            )
            m1_qd2, m2_qd2, m3_qd2 = (
                np.outer(m_i_Gamma_i_j[1], Gamma_2_qd2) + m_i_Kappa_i_j @ kappa_j_qe_d2
            )
            m1_qd3, m2_qd3, m3_qd3 = (
                np.outer(m_i_Gamma_i_j[2], Gamma_3_qd3) + m_i_Kappa_i_j @ kappa_j_qe_d3
            )
    ...
    def body_force_el(self, force, t, el):
        ...
        for i in range(self.nQP):
            ...
            fe[self.rDOF] += (
                NNi.T @ force(self.qp[el, i], t) * self.J0[el, i] * self.qw[el, i]
            )
        return fe
    ...
    def post_processing_subsystem(self, t, q, u, binary=True):
        ...
        Kappa_n[:, :, :2] = 0.0
        field_data = {
            "W_tot": self.integrate(
                self.material_model.potential, Gamma, self.Gamma0, Kappa, self.Kappa0
            ),
            "We_tot": self.integrate(
                self.material_model.potential,
                Gamma,
                self.Gamma0,
                self.Kappa0,
                self.Kappa0,
            ),
            "Wt_tot": self.integrate(
                self.material_model.potential,
                self.Gamma0,
                self.Gamma0,
                Kappa_t,
...
class Timoshenko_director_dirac(Timoshenko_beam_director):
    ...
    def g_q(self, t, q, coo):
        for i, DOF in enumerate(self.nodalDOF_di):
            ...
            coo.extend(
                self.__g_q(q[DOF]),
                (self.la_gDOF[np.arange(idx, idx + 6)], self.qDOF[DOF]),
            )
    ...
    def W_g(self, t, q, coo):
        for i, DOF in enumerate(self.nodalDOF_di):
            ...
            coo.extend(
                self.__g_q(q[DOF]).T,
                (self.uDOF[DOF], self.la_gDOF[np.arange(idx, idx + 6)]),
            )
    ...
    def Wla_g_q(self, t, q, la_g, coo):
        for i, DOF in enumerate(self.nodalDOF_di):
            idx = i * 6
            coo.extend(
                np.einsum("i,ijk->jk", la_g[idx : idx + 6], self.__g_qq()),
                (self.uDOF[DOF], self.qDOF[DOF]),
            )
