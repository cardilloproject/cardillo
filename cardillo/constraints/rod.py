...
class Rod:
    ...
    def g_q(self, t, q):
        r_OP1 = self.r_OP1(t, q)
        r_OP2 = self.r_OP2(t, q)
        r_OP1_q = self.r_OP1_q1(t, q)
        r_OP2_q = self.r_OP2_q2(t, q)
        return np.array(
    ...
    def g_dot_q(self, t, q, u):
        ...
        g_dot_q[:, : self.__nq1] = -2 * (
            r_P1P2 @ self.v_P1_q1(t, q, u) + v_P1P2 @ self.r_OP1_q1(t, q)
        )
        g_dot_q[:, self.__nq1 :] = 2 * (
            r_P1P2 @ self.v_P2_q2(t, q, u) + v_P1P2 @ self.r_OP2_q2(t, q)
        )
    ...
    def g_ddot_q(self, t, q, u, u_dot):
        ...
        g_ddot_q[:, : self.__nq1] = -2 * (
            2 * v_P1P2 @ self.v_P1_q1(t, q, u)
            + a_P1P2 @ self.r_OP1_q1(t, q)
            + r_P1P2 @ self.a_P1_q1(t, q, u, u_dot)
        )
        g_ddot_q[:, self.__nq1 :] = 2 * (
            2 * v_P1P2 @ self.v_P2_q2(t, q, u)
            + a_P1P2 @ self.r_OP2_q2(t, q)
            + r_P1P2 @ self.a_P2_q2(t, q, u, u_dot)
        )
    ...
    def g_ddot_u(self, t, q, u, u_dot):
        ...
        g_ddot_u[:, : self.__nu1] = -2 * (
            2 * v_P1P2 @ self.J_P1(t, q) + r_P1P2 @ self.a_P1_u1(t, q, u, u_dot)
        )
        g_ddot_u[:, self.__nu1 :] = 2 * (
            2 * v_P1P2 @ self.J_P2(t, q) + r_P1P2 @ self.a_P2_u2(t, q, u, u_dot)
        )
    ...
    def Wla_g_q(self, t, q, la_g):
        ...
        nu1 = self.__nu1
        r_P1P2 = self.r_OP2(t, q) - self.r_OP1(t, q)
        r_OP1_q1 = self.r_OP1_q1(t, q)
        r_OP2_q2 = self.r_OP2_q2(t, q)
        J_P1 = self.J_P1(t, q)
        J_P2 = self.J_P2(t, q)
        J_P1_q1 = self.J_P1_q1(t, q)
        J_P2_q2 = self.J_P2_q2(t, q)

        # dense blocks
        Wla_g_q = np.zeros((self.__nu, self.__nq))
        Wla_g_q[:nu1, :nq1] = (
            2 * la_g * (J_P1.T @ r_OP1_q1 - np.einsum("i,ijk->jk", r_P1P2, J_P1_q1))
        )
        Wla_g_q[:nu1, nq1:] = -2 * la_g * J_P1.T @ r_OP2_q2
        Wla_g_q[nu1:, :nq1] = -2 * la_g * J_P2.T @ r_OP1_q1
        Wla_g_q[nu1:, nq1:] = (
            2 * la_g * (J_P2.T @ r_OP2_q2 + np.einsum("i,ijk->jk", r_P1P2, J_P2_q2))
        )

        # Wla_g_q_num = approx_fprime(q, lambda q: self.W_g(t,q)*la_g)
        # error = np.linalg.norm(Wla_g_q-Wla_g_q_num)
