...
class Single_position_y:
    def __init__(
        ...
        # 2 position dof in exB and ezB direction + free rotation
        self.nla_g = 1
...
class Single_position_all_angles:
    def __init__(
        self,
        subsystem1,
        subsystem2,
        ...
        # 2 position dof in exB and ezB direction
        self.nla_g = 4
        self.la_g0 = np.zeros(self.nla_g) if la_g0 is None else la_g0
    ...
    def g(self, t, q):
        ...
        return np.array([r_P1P2 @ ey1, ex1 @ ey2, ex1 @ ez2, ez1 @ ex2])
    ...
    def Wla_g_q(self, t, q, la_g, coo):
        ...
        dense[:nu1, :nq1] += np.einsum("i,ijk->jk", la_g[1] * n, J_R1_q) + np.einsum(
            "ij,ik->kj", la_g[2] * n_q1, J_R1
        )
        dense[:nu1, nq1:] += np.einsum("ij,ik->kj", la_g[1] * n_q2, J_R1)
        dense[nu1:, :nq1] += np.einsum("ij,ik->kj", -la_g[1] * n_q1, J_R2)
        dense[nu1:, nq1:] += np.einsum("i,ijk->jk", -la_g[1] * n, J_R2_q) + np.einsum(
