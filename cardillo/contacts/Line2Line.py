...
class Line2Line:
    ...
    def f_pot(self, t, q):
        ...
        for el in range(subsystem1.nelement):
            for i in range(subsystem1.nquadrature):
                ...
                # #####################################
                # # closest point on complex boundaries
                # #####################################

                # # find closest point on second line, Meier2016c (35), (36)
                # eta, alpha, beta = self.closest_points_minimize_d2_boundary(t, q, xi)
                ...
                # # p1 = np.array([xi, alpha])
                # # x1_b = x1_b_fun(p1)
                # # # g1, g2 = approx_fprime(p1, x1_b_fun).T
                # # # g3 = cross3(g1, g2)
                # # # norm_g3 = norm(g3)
                # # # n1 = g3 / norm_g3 if norm_g3 > 0. else g3
                ...
                #     r2_eta = self.r2_eta(t, q, eta)
                #     R2 = rodriguez_B(e1, r2_eta)
                #     return r2 + self.R2(eta, beta) * (
                #         cos(beta) * R2 @ e2 + sin(beta) * R2 @ e3
                #     )

                # p2 = np.array([eta, beta])
                # x2_b = x2_b_fun(p2)
                # g1, g2 = approx_fprime(p2, x2_b_fun).T
                # g3 = cross3(g1, g2)
                # norm_g3 = norm(g3)
                # n2 = g3 / norm_g3 if norm_g3 > 0.0 else g3
                # n2 = -n2
                ...
                # # gap function, Meier2016c (37)
                # # TODO: how can we get this done without substractng a small
                # #       value in combination with smooth contacts?
                # # g = d
                # g = d - (self.R1(xi, alpha) + self.R2(eta, beta)) * 0.1
                ...
                f[: self.nu1][uDOF1] -= J1.T @ f_c * subsystem1.qw[el, i]
                # - subsystem 2 contribution
                f[self.nu1 :][uDOF2] += J2.T @ f_c * subsystem1.qw[el, i]
    ...
    def f_pot_q(self, t, q, coo):
        dense = approx_fprime(q, lambda q: self.f_pot(t, q))
