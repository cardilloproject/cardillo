...
class First_gradient():
    ...
    # line forces
    ####################################################
    def force_distr1D_el(self, force, t, el, edge_mesh):
        fe = np.zeros(edge_mesh.nq_el)

        for i in range(edge_mesh.nqp):
            N = edge_mesh.N[el, i]
            w_J0 = self.edge_w_J0[edge_mesh.idx][el, i]
            
            xi = edge_mesh.qp_xi[el, i]

            # internal forces
            for a in range(edge_mesh.nn_el):
                fe[edge_mesh.nodalDOF[a]] += force(t, xi) * N[a] * w_J0

        return fe

    def force_distr1D(self, t, q, force, edge_idx):
        z = self.z(t, q)
        f = np.zeros(self.nz)

        edge_mesh = self.mesh.edge_mesh[edge_idx]
        edge_zDOF = self.mesh.edge_qDOF[edge_idx].ravel()
        
        for el in range(edge_mesh.nel):
            f[edge_zDOF[edge_mesh.elDOF[el]]] += self.force_distr1D_el(force, t, el, edge_mesh)
        return f[self.fDOF]

    def force_distr_1D_q(self, t, q, coo, force, srf_idx):
        pass
