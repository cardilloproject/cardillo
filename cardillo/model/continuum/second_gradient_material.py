...
class Pantosheet_beam_network:
    ...
    def P(self, F, G):
        ...
        # Elongation
        Pe = self.Ke * (
            (rho1 - 1) * np.outer(e1, self.D1) + (rho2 - 1) * np.outer(e2, self.D2)
        )
        ...
        Psa = (
            self.Ks
            * np.arcsin(singa)
            * (np.outer(PsaD1, self.D1) + np.outer(PsaD2, self.D2))
        )
        ...
        # Normal curvature
        Pn1D1 = (
            (kn1 * tanga + e1 @ c1) * m1 + tanga * cross3(e1, cross3(c1, e1)) - kn1 * e1
        ) / rho1
        Pn1D2 = ((-kn1 * singa - cosga * e1 @ c1) * m2 - cross3(e2, cross3(c1, e2))) / (
            rho2 * cosga
        )

        Pn2D1 = ((kn2 * singa - cosga * e2 @ c2) * m1 + cross3(e1, cross3(c2, e1))) / (
            cosga * rho1
        )
        Pn2D2 = (
            (-kn2 * tanga + e2 @ c2) * m2
            - tanga * cross3(e2, cross3(c2, e2))
            - kn2 * e2
        ) / rho2

        Pn = self.Kn * (
            (
                np.outer(kn1 * Pn1D1 + kn2 * Pn2D1, self.D1)
        ...
        # Geodesic curvature
        Pg1D1 = (
            kg1 * singa * m1 + cross3(e1, cross3(cross3(e2, c1), e1)) - cosga * kg1 * e1
        ) / (rho1 * cosga)
        Pg1D2 = (-kg1 * singa * m2 - cross3(e2, cross3(cross3(e1, c1), e2))) / (
            rho2 * cosga
        )

        Pg2D1 = (kg2 * singa * m1 + cross3(e1, cross3(cross3(e2, c2), e1))) / (
            rho1 * cosga
        )
        Pg2D2 = (
            -kg2 * singa * m2
            - cross3(e2, cross3(cross3(e1, c2), e2))
            - cosga * kg2 * e2
        ) / (rho2 * cosga)

        Pg = self.Kg * (
            (
                np.outer(kg1 * Pg1D1 + kg2 * Pg2D1, self.D1)
        ...
        # Torsion
        Pt1D1 = (
            (2 * tau1 * singa + kg1 * cosga) * m1
            - kg1 * singa * e1
            - cross3(e1, cross3(cross3(e2, g1 - singa * c1), e1)) / cosga
        ) / (rho1 * cosga)
        Pt1D2 = (
            (-2 * tau1 * singa - kg1 * cosga) * m2
            + (g1 @ na) * e2
            + cross3(e2, cross3(cross3(e1, g1 - singa * c1), e2)) / cosga
        ) / (rho2 * cosga)

        Pt2D1 = (
            (2 * tau2 * singa + kg2 * cosga) * m1
            + (g2 @ na) * e1
            - cross3(e1, cross3(cross3(e2, g2 - singa * c2), e1)) / cosga
        ) / (rho1 * cosga)
        Pt2D2 = (
            (-2 * tau2 * singa - kg2 * cosga) * m2
            - kg2 * singa * e2
            + cross3(e2, cross3(cross3(e1, g2 - singa * c2), e2)) / cosga
        ) / (rho2 * cosga)

        Pt = self.Kt * (
            np.outer(tau1 * Pt1D1 + tau2 * Pt2D1, self.D1)
    ...
    def bbP(self, F, G):
        ...
        bbPt1 = (
            ...
            * tau1
            * (
                tanga * np.einsum("i,j,k->ijk", na, self.D1, self.D1) / rho1
        ...
        bbPt2 = (
            ...
            * tau2
            * (
                tanga * np.einsum("i,j,k->ijk", na, self.D2, self.D2) / rho2
    ...
    def W(self, F, G):
        ...
        m2 = cross3(na, e2)
        c1 = (
            np.einsum("ijk,j,k->i", G, self.D1, self.D1) / rho1
        )  # same as G@D1@D1 speed is similar
        c2 = np.einsum("ijk,j,k->i", G, self.D2, self.D2) / rho2
        g1 = np.einsum("ijk,j,k->i", G, self.D2, self.D1) / rho2
        g2 = np.einsum("ijk,j,k->i", G, self.D1, self.D2) / rho1
        ...
        # complete energy
        W = (
            0.5 * self.Ke * ((rho1 - 1) ** 2 + (rho2 - 1) ** 2)
            + 0.5 * self.Kg * ((na @ c1) ** 2 + (na @ c2) ** 2)
            + 0.5 * self.Kn * ((-m1 @ c1) ** 2 + (-m2 @ c2) ** 2)
            + 0.5 * self.Kt * (tau1**2 + tau2**2)
            + 0.5 * self.Ks * (np.arcsin(singa) ** 2)
        )
    ...
    def We(self, F, G):
        ...
        We = 0.5 * self.Ke * ((rho1 - 1) ** 2 + (rho2 - 1) ** 2)
        return We
...
class Pantobox_beam_network:
    ...
    def __init__(
        ...
        self.lc3 = np.array(
            [
                [[LeviCivita3(i, j, k) for k in range(3)] for j in range(3)]
                for i in range(3)
            ]
        )
    ...
    def P(self, F, G=G0):
        ...
        # Elongation
        Pe = self.Ke * (
            (rho1 - 1) * np.outer(e1, self.D1)
            + (rho2 - 1) * np.outer(e2, self.D2)
            + (rho3 - 1) * np.outer(e3, self.D3)
            + (rho4 - 1) * np.outer(e4, self.D4)
        )
        ...
        Psb = (
            self.Ks
            * np.arcsin(singb)
            * (np.outer(PsbD3, self.D3) + np.outer(PsbD4, self.D4))
        )

        PcD1 = (
            singc * tanga * m1 + cross3(e1, cross3(cross3(e2, nb), e1)) / cosga
        ) / rho1
        PcD2 = (
            -singc * tanga * m2 - cross3(e2, cross3(cross3(e1, nb), e2)) / cosga
        ) / rho2
        PcD3 = (
            singc * tangb * m3 + cross3(e3, cross3(cross3(e4, na), e3)) / cosgb
        ) / rho3
        PcD4 = (
            -singc * tangb * m4 - cross3(e4, cross3(cross3(e3, na), e4)) / cosgb
        ) / rho4
        Pc = (
            self.Kc
            * np.arcsin(singc)
            / cosgc
            * (
                np.outer(PcD1, self.D1)
                + np.outer(PcD2, self.D2)
                + np.outer(PcD3, self.D3)
                + np.outer(PcD4, self.D4)
            )
        )
        ...
        Pn3D3 = (
            (kn3 * tangb + e3 @ c3) * m3 + tangb * cross3(e3, cross3(c3, e3)) - kn3 * e3
        ) / rho3
        Pn3D4 = ((-kn3 * singb - cosgb * e3 @ c3) * m4 - cross3(e4, cross3(c3, e4))) / (
            rho4 * cosgb
        )

        Pn4D3 = ((kn4 * singb - cosgb * e4 @ c4) * m3 + cross3(e3, cross3(c4, e3))) / (
            cosgb * rho3
        )
        Pn4D4 = (
            (-kn4 * tangb + e4 @ c4) * m4
            - tangb * cross3(e4, cross3(c4, e4))
            - kn4 * e4
        ) / rho4
        ...
        Pg3D3 = (
            kg3 * singb * m3 + cross3(e3, cross3(cross3(e4, c3), e3)) - cosgb * kg3 * e3
        ) / (rho3 * cosgb)
        Pg3D4 = (-kg3 * singb * m4 - cross3(e4, cross3(cross3(e3, c3), e4))) / (
            rho4 * cosgb
        )

        Pg4D3 = (kg4 * singb * m3 + cross3(e3, cross3(cross3(e4, c4), e3))) / (
            rho3 * cosgb
        )
        Pg4D4 = (
            -kg4 * singb * m4
            - cross3(e4, cross3(cross3(e3, c4), e4))
            - cosgb * kg4 * e4
        ) / (rho4 * cosgb)
        ...
        Pt3D3 = (
            (2 * tau3 * singb + kg3 * cosgb) * m3
            - kg3 * singb * e3
            - cross3(e3, cross3(cross3(e4, g3 - singb * c3), e3)) / cosgb
        ) / (rho3 * cosgb)
        Pt3D4 = (
            (-2 * tau3 * singb - kg3 * cosgb) * m4
            + (g3 @ nb) * e4
            + cross3(e4, cross3(cross3(e3, g3 - singb * c3), e4)) / cosgb
        ) / (rho4 * cosgb)

        Pt4D3 = (
            (2 * tau4 * singb + kg4 * cosgb) * m3
            + (g4 @ nb) * e3
            - cross3(e3, cross3(cross3(e4, g4 - singb * c4), e3)) / cosgb
        ) / (rho3 * cosgb)
        Pt4D4 = (
            (-2 * tau4 * singb - kg4 * cosgb) * m4
            - kg4 * singb * e4
            + cross3(e4, cross3(cross3(e3, g4 - singb * c4), e4)) / cosgb
        ) / (rho4 * cosgb)
    ...
    def P_F(self, F, G=G0):
        ...
        e1_F = (
            np.einsum("ij,k->ijk", self.I3, self.D1) / rho1
            - np.einsum("i,j,k->ijk", e1, e1, self.D1) / rho1
        )
        e2_F = (
            np.einsum("ij,k->ijk", self.I3, self.D2) / rho2
            - np.einsum("i,j,k->ijk", e2, e2, self.D2) / rho2
        )
        e3_F = (
            np.einsum("ij,k->ijk", self.I3, self.D3) / rho3
            - np.einsum("i,j,k->ijk", e3, e3, self.D3) / rho3
        )
        e4_F = (
            np.einsum("ij,k->ijk", self.I3, self.D4) / rho4
        ...
        e1_F_F = (
            -(
                np.einsum("ijk,lm->ijklm", e1_F, rho1_F)
                + np.einsum("ijk,lm->ilmjk", e1_F, rho1_F)
        ...
        e2_F_F = (
            -(
                np.einsum("ijk,lm->ijklm", e2_F, rho2_F)
                + np.einsum("ijk,lm->ilmjk", e2_F, rho2_F)
        ...
        e3_F_F = (
            -(
                np.einsum("ijk,lm->ijklm", e3_F, rho3_F)
                + np.einsum("ijk,lm->ilmjk", e3_F, rho3_F)
        ...
        e4_F_F = (
            -(
                np.einsum("ijk,lm->ijklm", e4_F, rho4_F)
                + np.einsum("ijk,lm->ilmjk", e4_F, rho4_F)
                + np.einsum("i,jklm->ijklm", e4, rho4_F_F)
            )
            / rho4
        )

        ga_F = np.outer(m1, self.D1) / rho1 - np.outer(m2, self.D2) / rho2

        gb_F = np.outer(m3, self.D3) / rho3 - np.outer(m4, self.D4) / rho4

        m1_F = (
            np.einsum("i,jk->ijk", tanga * m1 - e1, ga_F) + e2_F / cosga - tanga * e1_F
        )
        m2_F = (
            np.einsum("i,jk->ijk", tanga * m2 + e2, ga_F) - e1_F / cosga + tanga * e2_F
        )
        m3_F = (
            np.einsum("i,jk->ijk", tangb * m3 - e3, gb_F) + e4_F / cosgb - tangb * e3_F
        )
        m4_F = (
            np.einsum("i,jk->ijk", tangb * m4 + e4, gb_F) - e3_F / cosgb + tangb * e4_F
        )

        ga_F_F = (
            -np.einsum("i,j,kl->ijkl", m1, self.D1, rho1_F) / rho1**2
        ...
        gb_F_F = (
            -np.einsum("i,j,kl->ijkl", m3, self.D3, rho3_F) / rho3**2
        ...
        m1_F_F = (
            np.einsum(
                "ilm,jk->ijklm",
                np.einsum("i,lm->ilm", m1 / cosga**2, ga_F) + tanga * m1_F - e1_F,
                ga_F,
            )
        ...
        m2_F_F = (
            np.einsum(
                "ilm,jk->ijklm",
                np.einsum("i,lm->ilm", m2 / cosga**2, ga_F) + tanga * m2_F + e2_F,
                ga_F,
            )
        ...
        m3_F_F = (
            np.einsum(
                "ilm,jk->ijklm",
                np.einsum("i,lm->ilm", m3 / cosgb**2, gb_F) + tangb * m3_F - e3_F,
                gb_F,
            )
        ...
        m4_F_F = (
            np.einsum(
                "ilm,jk->ijklm",
                np.einsum("i,lm->ilm", m4 / cosgb**2, gb_F) + tangb * m4_F + e4_F,
                gb_F,
            )
        ...
        # Elongation
        Pe1_F = np.einsum(
            "i,j,k,l->ijkl", self.Ke * e1, self.D1, e1, self.D1
        ) + np.einsum("i,jkl->jikl", self.Ke * self.D1 * (rho1 - 1), e1_F)
        Pe2_F = np.einsum(
            "i,j,k,l->ijkl", self.Ke * e2, self.D2, e2, self.D2
        ) + np.einsum("i,jkl->jikl", self.Ke * self.D2 * (rho2 - 1), e2_F)
        Pe3_F = np.einsum(
            "i,j,k,l->ijkl", self.Ke * e3, self.D3, e3, self.D3
        ) + np.einsum("i,jkl->jikl", self.Ke * self.D3 * (rho3 - 1), e3_F)
        Pe4_F = np.einsum(
            "i,j,k,l->ijkl", self.Ke * e4, self.D4, e4, self.D4
        ...
        # shear
        Psa_F = self.Ks * (
            np.arcsin(singa) * ga_F_F + np.einsum("jA,kB->jAkB", ga_F, ga_F)
        )
        Psb_F = self.Ks * (
            np.arcsin(singb) * gb_F_F + np.einsum("jA,kB->jAkB", gb_F, gb_F)
        )

        gc_F = np.einsum("i,ijk->jk", na / cosgc, nb_F) + np.einsum(
            "i,ijk->jk", nb / cosgc, na_F
        )
        gc_F_F = (
            np.einsum("i,ijAkB->jAkB", nb / cosgc, na_F_F)
        ...
        Pc_F = self.Kc * (
            np.arcsin(singc) * gc_F_F + np.einsum("jA,kB->jAkB", gc_F, gc_F)
        )
        ...
        Pn1_F = self.Kn * (
            ...
            + (m1 @ c1)
            * (
                np.einsum("ijklm,i->jklm", m1_F_F, c1)
                + np.einsum("ijk,ilm->jklm", m1_F, c1_F)
                + np.einsum("ilm,ijk->jklm", m1_F, c1_F)
        ...
        Pn2_F = self.Kn * (
            ...
            + (m2 @ c2)
            * (
                np.einsum("ijklm,i->jklm", m2_F_F, c2)
                + np.einsum("ijk,ilm->jklm", m2_F, c2_F)
                + np.einsum("ilm,ijk->jklm", m2_F, c2_F)
        ...
        Pn3_F = self.Kn * (
            ...
            + (m3 @ c3)
            * (
                np.einsum("ijklm,i->jklm", m3_F_F, c3)
                + np.einsum("ijk,ilm->jklm", m3_F, c3_F)
                + np.einsum("ilm,ijk->jklm", m3_F, c3_F)
        ...
        Pn4_F = self.Kn * (
            ...
            + (m4 @ c4)
            * (
                np.einsum("ijklm,i->jklm", m4_F_F, c4)
                + np.einsum("ijk,ilm->jklm", m4_F, c4_F)
                + np.einsum("ilm,ijk->jklm", m4_F, c4_F)
        ...
        Pg1_F = self.Kg * (
            ...
            + (na @ c1)
            * (
                np.einsum("ijklm,i->jklm", na_F_F, c1)
                + np.einsum("ijk,ilm->jklm", na_F, c1_F)
                + np.einsum("ilm,ijk->jklm", na_F, c1_F)
        ...
        Pg2_F = self.Kg * (
            ...
            + (na @ c2)
            * (
                np.einsum("ijklm,i->jklm", na_F_F, c2)
                + np.einsum("ijk,ilm->jklm", na_F, c2_F)
                + np.einsum("ilm,ijk->jklm", na_F, c2_F)
        ...
        Pg3_F = self.Kg * (
            ...
            + (nb @ c3)
            * (
                np.einsum("ijklm,i->jklm", nb_F_F, c3)
                + np.einsum("ijk,ilm->jklm", nb_F, c3_F)
                + np.einsum("ilm,ijk->jklm", nb_F, c3_F)
        ...
        Pg4_F = self.Kg * (
            ...
            + (nb @ c4)
            * (
                np.einsum("ijklm,i->jklm", nb_F_F, c4)
                + np.einsum("ijk,ilm->jklm", nb_F, c4_F)
                + np.einsum("ilm,ijk->jklm", nb_F, c4_F)
        ...
        tau1_F = (
            ga_F * (tau1 * tanga + na @ c1)
            + np.einsum("ijk,i->jk", -na_F, g1 / cosga - tanga * c1)
            + np.einsum("i,ijk->jk", -na, g1_F / cosga - tanga * c1_F)
        )
        tau2_F = (
            ga_F * (tau2 * tanga + na @ c2)
            + np.einsum("ijk,i->jk", -na_F, (g2 - singa * c2) / cosga)
            + np.einsum("i,ijk->jk", -na / cosga, g2_F)
            + np.einsum("i,ijk->jk", tanga * na, c2_F)
        )
        tau3_F = (
            gb_F * (tau3 * tangb + nb @ c3)
            + np.einsum("ijk,i->jk", -nb_F, (g3 - singb * c3) / cosgb)
            + np.einsum("i,ijk->jk", -nb / cosgb, g3_F)
            + np.einsum("i,ijk->jk", tangb * nb, c3_F)
        )
        tau4_F = (
            gb_F * (tau4 * tangb + nb @ c4)
            + np.einsum("ijk,i->jk", -nb_F, (g4 - singb * c4) / cosgb)
            + np.einsum("i,ijk->jk", -nb / cosgb, g4_F)
            + np.einsum("i,ijk->jk", tangb * nb, c4_F)
        )

        tau1_F_F = (
            ga_F_F * (tau1 * tanga + na @ c1)
            + np.einsum(
                "jA,kB->jAkB",
                ga_F,
                tau1_F * tanga
                + (tau1 / cosga**2) * ga_F
                + np.einsum("ikB,i->kB", na_F, c1)
                + np.einsum("ikB,i->kB", c1_F, na),
            )
            + np.einsum("ijAkB,i->jAkB", -na_F_F, g1 / cosga - tanga * c1)
            + np.einsum(
                "ijA,i,kB->jAkB", -na_F, (g1 * (tanga / cosga) - c1 / cosga**2), ga_F
            )
            + np.einsum("ijA,ikB->jAkB", -na_F, g1_F / cosga - tanga * c1_F)
            + np.einsum("ikB,ijA->jAkB", -na_F, g1_F / cosga - tanga * c1_F)
            + np.einsum(
                "i,ijA,kB->jAkB", -na, g1_F * (tanga / cosga) - c1_F / cosga**2, ga_F
            )
            + np.einsum("i,ijAkB->jAkB", -na, g1_F_F / cosga - tanga * c1_F_F)
        )
        tau2_F_F = (
            ga_F_F * (tau2 * tanga + na @ c2)
            + np.einsum(
                "jA,kB->jAkB",
                ga_F,
                tau2_F * tanga
                + (tau2 / cosga**2) * ga_F
                + np.einsum("ikB,i->kB", na_F, c2)
                + np.einsum("ikB,i->kB", c2_F, na),
            )
            + np.einsum("ijAkB,i->jAkB", -na_F_F, g2 / cosga - tanga * c2)
            + np.einsum(
                "ijA,i,kB->jAkB", -na_F, (g2 * (tanga / cosga) - c2 / cosga**2), ga_F
            )
            + np.einsum("ijA,ikB->jAkB", -na_F, g2_F / cosga - tanga * c2_F)
            + np.einsum("ikB,ijA->jAkB", -na_F, g2_F / cosga - tanga * c2_F)
            + np.einsum(
                "i,ijA,kB->jAkB", -na, g2_F * (tanga / cosga) - c2_F / cosga**2, ga_F
            )
            + np.einsum("i,ijAkB->jAkB", -na, g2_F_F / cosga - tanga * c2_F_F)
        )
        tau3_F_F = (
            gb_F_F * (tau3 * tangb + nb @ c3)
            + np.einsum(
                "jA,kB->jAkB",
                gb_F,
                tau3_F * tangb
                + (tau3 / cosgb**2) * gb_F
                + np.einsum("ikB,i->kB", nb_F, c3)
                + np.einsum("ikB,i->kB", c3_F, nb),
            )
            + np.einsum("ijAkB,i->jAkB", -nb_F_F, g3 / cosgb - tangb * c3)
            + np.einsum(
                "ijA,i,kB->jAkB", -nb_F, (g3 * (tangb / cosgb) - c3 / cosgb**2), gb_F
            )
            + np.einsum("ijA,ikB->jAkB", -nb_F, g3_F / cosgb - tangb * c3_F)
            + np.einsum("ikB,ijA->jAkB", -nb_F, g3_F / cosgb - tangb * c3_F)
            + np.einsum(
                "i,ijA,kB->jAkB", -nb, g3_F * (tangb / cosgb) - c3_F / cosgb**2, gb_F
            )
            + np.einsum("i,ijAkB->jAkB", -nb, g3_F_F / cosgb - tangb * c3_F_F)
        )
        tau4_F_F = (
            gb_F_F * (tau4 * tangb + nb @ c4)
            + np.einsum(
                "jA,kB->jAkB",
                gb_F,
                tau4_F * tangb
                + (tau4 / cosgb**2) * gb_F
                + np.einsum("ikB,i->kB", nb_F, c4)
                + np.einsum("ikB,i->kB", c4_F, nb),
            )
            + np.einsum("ijAkB,i->jAkB", -nb_F_F, g4 / cosgb - tangb * c4)
            + np.einsum(
                "ijA,i,kB->jAkB", -nb_F, (g4 * (tangb / cosgb) - c4 / cosgb**2), gb_F
            )
            + np.einsum("ijA,ikB->jAkB", -nb_F, g4_F / cosgb - tangb * c4_F)
            + np.einsum("ikB,ijA->jAkB", -nb_F, g4_F / cosgb - tangb * c4_F)
            + np.einsum(
                "i,ijA,kB->jAkB", -nb, g4_F * (tangb / cosgb) - c4_F / cosgb**2, gb_F
            )
            + np.einsum("i,ijAkB->jAkB", -nb, g4_F_F / cosgb - tangb * c4_F_F)
        )

        Pt1_F = (
            np.einsum("iA,kB->iAkB", self.Kt * tau1_F, tau1_F)
            + self.Kt * tau1 * tau1_F_F
        )
        Pt2_F = (
            np.einsum("iA,kB->iAkB", self.Kt * tau2_F, tau2_F)
            + self.Kt * tau2 * tau2_F_F
        )
        Pt3_F = (
            np.einsum("iA,kB->iAkB", self.Kt * tau3_F, tau3_F)
            + self.Kt * tau3 * tau3_F_F
        )
        Pt4_F = (
            np.einsum("iA,kB->iAkB", self.Kt * tau4_F, tau4_F)
            + self.Kt * tau4 * tau4_F_F
        )
    ...
    def P_G(self, F, G=G0):
        ...
        na_F = (
            np.einsum("i,jk->ijk", tanga * na, ga_F)
            + np.cross(e1_F, e2 / cosga, axis=0)
            - np.cross(e2_F, e1 / cosga, axis=0)
        )
        nb_F = (
            np.einsum("i,jk->ijk", tangb * nb, gb_F)
            + np.cross(e3_F, e4 / cosgb, axis=0)
            - np.cross(e4_F, e3 / cosgb, axis=0)
        )
        ...
        c1_F_G = -np.einsum(
            "ik,j,A,B,C->ijAkBC", self.I3, e1, self.D1, self.D1, self.D1 / rho1**2
        )
        c2_F_G = -np.einsum(
            "ik,j,A,B,C->ijAkBC", self.I3, e2, self.D2, self.D2, self.D2 / rho2**2
        )
        c3_F_G = -np.einsum(
            "ik,j,A,B,C->ijAkBC", self.I3, e3, self.D3, self.D3, self.D3 / rho3**2
        )
        c4_F_G = -np.einsum(
            "ik,j,A,B,C->ijAkBC", self.I3, e4, self.D4, self.D4, self.D4 / rho4**2
        )
        ...
        g1_F_G = -np.einsum(
            "ik,j,A,B,C->ijAkBC", self.I3, e2, self.D2, self.D2, self.D1 / rho2**2
        )
        g2_F_G = -np.einsum(
            "ik,j,A,B,C->ijAkBC", self.I3, e1, self.D1, self.D1, self.D2 / rho1**2
        )
        g3_F_G = -np.einsum(
            "ik,j,A,B,C->ijAkBC", self.I3, e4, self.D4, self.D4, self.D3 / rho4**2
        )
        g4_F_G = -np.einsum(
            "ik,j,A,B,C->ijAkBC", self.I3, e3, self.D3, self.D3, self.D4 / rho3**2
        )
        ...
        kn1_F_G = np.einsum("ijA,ikBC->jAkBC", m1_F, c1_G) + np.einsum(
            "ijAkBC,i->jAkBC", c1_F_G, m1
        )
        kn2_F_G = np.einsum("ijA,ikBC->jAkBC", m2_F, c2_G) + np.einsum(
            "ijAkBC,i->jAkBC", c2_F_G, m2
        )
        kn3_F_G = np.einsum("ijA,ikBC->jAkBC", m3_F, c3_G) + np.einsum(
            "ijAkBC,i->jAkBC", c3_F_G, m3
        )
        kn4_F_G = np.einsum("ijA,ikBC->jAkBC", m4_F, c4_G) + np.einsum(
            "ijAkBC,i->jAkBC", c4_F_G, m4
        )

        Pn1_G = (
            np.einsum("i,ikBC,jA->jAkBC", self.Kn * m1, c1_G, kn1_F)
            + self.Kn * m1 @ c1 * kn1_F_G
        )
        Pn2_G = (
            np.einsum("i,ikBC,jA->jAkBC", self.Kn * m2, c2_G, kn2_F)
            + self.Kn * m2 @ c2 * kn2_F_G
        )
        Pn3_G = (
            np.einsum("i,ikBC,jA->jAkBC", self.Kn * m3, c3_G, kn3_F)
            + self.Kn * m3 @ c3 * kn3_F_G
        )
        Pn4_G = (
            np.einsum("i,ikBC,jA->jAkBC", self.Kn * m4, c4_G, kn4_F)
        ...
        kg1_F_G = np.einsum("ijA,ikBC->jAkBC", na_F, c1_G) + np.einsum(
            "ijAkBC,i->jAkBC", c1_F_G, na
        )
        kg2_F_G = np.einsum("ijA,ikBC->jAkBC", na_F, c2_G) + np.einsum(
            "ijAkBC,i->jAkBC", c2_F_G, na
        )
        kg3_F_G = np.einsum("ijA,ikBC->jAkBC", nb_F, c3_G) + np.einsum(
            "ijAkBC,i->jAkBC", c3_F_G, nb
        )
        kg4_F_G = np.einsum("ijA,ikBC->jAkBC", nb_F, c4_G) + np.einsum(
            "ijAkBC,i->jAkBC", c4_F_G, nb
        )

        Pg1_G = (
            np.einsum("i,ikBC,jA->jAkBC", self.Kg * na, c1_G, kg1_F)
            + self.Kg * na @ c1 * kg1_F_G
        )
        Pg2_G = (
            np.einsum("i,ikBC,jA->jAkBC", self.Kg * na, c2_G, kg2_F)
            + self.Kg * na @ c2 * kg2_F_G
        )
        Pg3_G = (
            np.einsum("i,ikBC,jA->jAkBC", self.Kg * nb, c3_G, kg3_F)
            + self.Kg * nb @ c3 * kg3_F_G
        )
        Pg4_G = (
            np.einsum("i,ikBC,jA->jAkBC", self.Kg * nb, c4_G, kg4_F)
        ...
        tau1_F = (
            ga_F * (tau1 * tanga + na @ c1)
            + np.einsum("ijA,i->jA", -na_F, (g1 - singa * c1) / cosga)
        ...
        tau1_F_G = (
            np.einsum(
                "jA,kBC->jAkBC",
                ga_F,
                tau1_G * tanga + np.einsum("i,ijAB->jAB", na, c1_G),
            )
        ...
        tau2_F_G = (
            np.einsum(
                "jA,kBC->jAkBC",
                ga_F,
                tau2_G * tanga + np.einsum("i,ijAB->jAB", na, c2_G),
            )
        ...
        tau3_F_G = (
            np.einsum(
                "jA,kBC->jAkBC",
                gb_F,
                tau3_G * tangb + np.einsum("i,ijAB->jAB", nb, c3_G),
            )
        ...
        tau4_F_G = (
            np.einsum(
                "jA,kBC->jAkBC",
                gb_F,
                tau4_G * tangb + np.einsum("i,ijAB->jAB", nb, c4_G),
            )
        ...
        Pt1_G = self.Kt * tau1 * tau1_F_G + np.einsum(
            "jA,kBC->jAkBC", self.Kt * tau1_F, tau1_G
        )
        Pt2_G = self.Kt * tau2 * tau2_F_G + np.einsum(
            "jA,kBC->jAkBC", self.Kt * tau2_F, tau2_G
        )
        Pt3_G = self.Kt * tau3 * tau3_F_G + np.einsum(
            "jA,kBC->jAkBC", self.Kt * tau3_F, tau3_G
        )
        Pt4_G = self.Kt * tau4 * tau4_F_G + np.einsum(
            "jA,kBC->jAkBC", self.Kt * tau4_F, tau4_G
        )
    ...
    def bbP(self, F, G):
        ...
        bbPt3 = (
            ...
            * tau3
            * (
                tangb * np.einsum("i,j,k->ijk", nb, self.D3, self.D3) / rho3
        ...
        bbPt4 = (
            ...
            * tau4
            * (
                tangb * np.einsum("i,j,k->ijk", nb, self.D4, self.D4) / rho4
    ...
    def bbP_F(self, F, G):
        ...
        bbPg1_F = (
            np.einsum("kC,j,A,B->jABkC", kg1_F, na, self.D1, self.D1 * self.Kg / rho1)
            + np.einsum(
                "ikC,A,B->iABkC", na_F, self.D1, self.D1 * (na @ c1) * self.Kg / rho1
            )
            - np.einsum(
                "kC,j,A,B->jABkC",
                rho1_F,
        ...
        bbPg2_F = (
            np.einsum("kC,j,A,B->jABkC", kg2_F, na, self.D2, self.D2 * self.Kg / rho2)
            + np.einsum(
                "ikC,A,B->iABkC", na_F, self.D2, self.D2 * (na @ c2) * self.Kg / rho2
            )
            - np.einsum(
                "kC,j,A,B->jABkC",
                rho2_F,
        ...
        bbPg3_F = (
            np.einsum("kC,j,A,B->jABkC", kg3_F, nb, self.D3, self.D3 * self.Kg / rho3)
            + np.einsum(
                "ikC,A,B->iABkC", nb_F, self.D3, self.D3 * (nb @ c3) * self.Kg / rho3
            )
            - np.einsum(
                "kC,j,A,B->jABkC",
                rho3_F,
        ...
        bbPg4_F = (
            np.einsum("kC,j,A,B->jABkC", kg4_F, nb, self.D4, self.D4 * self.Kg / rho4)
            + np.einsum(
                "ikC,A,B->iABkC", nb_F, self.D4, self.D4 * (nb @ c4) * self.Kg / rho4
            )
            - np.einsum(
                "kC,j,A,B->jABkC",
                rho4_F,
        ...
        bbPt1_F = (
            np.einsum(
                ...
                na,
                self.D1,
                self.D1 * (self.Kt / rho1),
                tau1_F * tanga
                + ga_F * (tau1 / cosga**2)
                - rho1_F * (tau1 * tanga / rho1),
            )
            + np.einsum(
                "ikC,A,B->iABkC",
                na_F,
                self.D1,
                self.D1 * (self.Kt * tau1 * tanga / rho1),
            )
            - np.einsum(
                "i,A,B,kC->iABkC",
                na,
                self.D2,
                self.D1 * (self.Kt / (rho2 * cosga)),
        ...
        bbPt2_F = (
            np.einsum(
                ...
                na,
                self.D2,
                self.D2 * (self.Kt / rho2),
                tau2_F * tanga
                + ga_F * (tau2 / cosga**2)
                - rho2_F * (tau2 * tanga / rho2),
            )
            + np.einsum(
                "ikC,A,B->iABkC",
                na_F,
                self.D2,
                self.D2 * (self.Kt * tau2 * tanga / rho2),
            )
            - np.einsum(
                "i,A,B,kC->iABkC",
                na,
                self.D1,
                self.D2 * (self.Kt / (rho1 * cosga)),
        ...
        bbPt3_F = (
            np.einsum(
                ...
                nb,
                self.D3,
                self.D3 * (self.Kt / rho3),
                tau3_F * tangb
                + gb_F * (tau3 / cosgb**2)
                - rho3_F * (tau3 * tangb / rho3),
            )
            + np.einsum(
                "ikC,A,B->iABkC",
                nb_F,
                self.D3,
                self.D3 * (self.Kt * tau3 * tangb / rho3),
            )
            - np.einsum(
                "i,A,B,kC->iABkC",
                nb,
                self.D4,
                self.D3 * (self.Kt / (rho4 * cosgb)),
        ...
        bbPt4_F = (
            np.einsum(
                ...
                nb,
                self.D4,
                self.D4 * (self.Kt / rho4),
                tau4_F * tangb
                + gb_F * (tau4 / cosgb**2)
                - rho4_F * (tau4 * tangb / rho4),
            )
            + np.einsum(
                "ikC,A,B->iABkC",
                nb_F,
                self.D4,
                self.D4 * (self.Kt * tau4 * tangb / rho4),
            )
            - np.einsum(
                "i,A,B,kC->iABkC",
                nb,
                self.D3,
                self.D4 * (self.Kt / (rho3 * cosgb)),
        ...
        bbPn1_F = (
            np.einsum("kC,j,A,B->jABkC", kn1_F, m1, self.D1, self.D1 * self.Kn / rho1)
            + np.einsum(
                "ikC,A,B->iABkC", m1_F, self.D1, self.D1 * (m1 @ c1) * self.Kn / rho1
            )
            - np.einsum(
                "kC,j,A,B->jABkC",
                rho1_F,
        ...
        bbPn2_F = (
            np.einsum("kC,j,A,B->jABkC", kn2_F, m2, self.D2, self.D2 * self.Kn / rho2)
            + np.einsum(
                "ikC,A,B->iABkC", m2_F, self.D2, self.D2 * (m2 @ c2) * self.Kn / rho2
            )
            - np.einsum(
                "kC,j,A,B->jABkC",
                rho2_F,
        ...
        bbPn3_F = (
            np.einsum("kC,j,A,B->jABkC", kn3_F, m3, self.D3, self.D3 * self.Kn / rho3)
            + np.einsum(
                "ikC,A,B->iABkC", m3_F, self.D3, self.D3 * (m3 @ c3) * self.Kn / rho3
            )
            - np.einsum(
                "kC,j,A,B->jABkC",
                rho3_F,
        ...
        bbPn4_F = (
            np.einsum("kC,j,A,B->jABkC", kn4_F, m4, self.D4, self.D4 * self.Kn / rho4)
            + np.einsum(
                "ikC,A,B->iABkC", m4_F, self.D4, self.D4 * (m4 @ c4) * self.Kn / rho4
            )
            - np.einsum(
                "kC,j,A,B->jABkC",
                rho4_F,
    ...
    def bbP_G(self, F, G):
        ...
        # normal bending
        bbPn1_G = np.einsum(
            "i,ikCD,j,A,B->jABkCD", m1, c1_G, m1, self.D1, self.D1 * self.Kn / rho1
        )
        bbPn2_G = np.einsum(
            "i,ikCD,j,A,B->jABkCD", m2, c2_G, m2, self.D2, self.D2 * self.Kn / rho2
        )
        bbPn3_G = np.einsum(
            "i,ikCD,j,A,B->jABkCD", m3, c3_G, m3, self.D3, self.D3 * self.Kn / rho3
        )
        bbPn4_G = np.einsum(
            "i,ikCD,j,A,B->jABkCD", m4, c4_G, m4, self.D4, self.D4 * self.Kn / rho4
        )
        ...
        # geodesic bending
        bbPg1_G = np.einsum(
            "i,ikCD,j,A,B->jABkCD", na, c1_G, na, self.D1, self.D1 * self.Kg / rho1
        )
        bbPg2_G = np.einsum(
            "i,ikCD,j,A,B->jABkCD", na, c2_G, na, self.D2, self.D2 * self.Kg / rho2
        )
        bbPg3_G = np.einsum(
            "i,ikCD,j,A,B->jABkCD", nb, c3_G, nb, self.D3, self.D3 * self.Kg / rho3
        )
        bbPg4_G = np.einsum(
            "i,ikCD,j,A,B->jABkCD", nb, c4_G, nb, self.D4, self.D4 * self.Kg / rho4
        )
    ...
    def e_F_F(self, F, G, al):
        ...
        e_F_F = (
            -(
                np.einsum("ijk,lm->ijklm", self.e_F(F, G, al), self.rho_F(F, G, al))
                + np.einsum("ijk,lm->ilmjk", self.e_F(F, G, al), self.rho_F(F, G, al))
    ...
    def ga_F_F(self, F, G):
        ...
        ga_F_F = (
            -np.einsum("i,j,kl->ijkl", m1, self.D1, self.rho_F(F, G, 0)) / rho1**2
    ...
    def m_F_F(self, F, G, al):
        ...
        m1_F_F = (
            np.einsum(
                "ilm,jk->ijklm",
                np.einsum("i,lm->ilm", m[0] / cosga**2, self.ga_F(F, G))
                + tanga * self.m_F(F, G, 0)
                - self.e_F(F, G, 0),
                self.ga_F(F, G),
            )
            ...
            + np.einsum(
                "ijk,lm->ijklm",
                (tanga * self.e_F(F, G, 1) - self.e_F(F, G, 0) / cosga) / cosga,
                self.ga_F(F, G),
            )
            + self.e_F_F(F, G, 1) / cosga
            - tanga * self.e_F_F(F, G, 0)
        )
    ...
    def na_F_F(self, F, G):
        ...
        na_F_F = (
            np.einsum("i,jk,lm", na / cosga**2, self.ga_F(F, G), self.ga_F(F, G))
            + np.einsum("i,jklm->ijklm", tanga * na, self.ga_F_F(F, G))
            + np.einsum("ilm,jk->ijklm", self.na_F(F, G), self.ga_F(F, G) * tanga)
            + np.einsum(
                "ijk,lm->ijklm",
                np.cross(self.e_F(F, G, 0), e2 / cosga, axis=0)
                - np.cross(self.e_F(F, G, 1), e1 / cosga, axis=0),
                tanga * self.ga_F(F, G),
            )
    ...
    def c_F_F(self, F, G):
        ...
        c1_F_F = np.einsum("i,j,k,lm", c1, e1 / rho1**2, self.D1, rho1_F) - np.einsum(
            "ilm, j, k->ijklm", c1_F, e1, self.D1 / rho1
    ...
    def W(self, F, G=G0):
        ...
        W = (
            0.5
            * self.Ke
            * ((rho1 - 1) ** 2 + (rho2 - 1) ** 2 + (rho3 - 1) ** 2 + (rho4 - 1) ** 2)
            + 0.5
            * self.Kg
            * ((na @ c1) ** 2 + (na @ c2) ** 2 + (nb @ c3) ** 2 + (nb @ c4) ** 2)
            + 0.5
            * self.Kn
            * ((-m1 @ c1) ** 2 + (-m2 @ c2) ** 2 + (-m3 @ c3) ** 2 + (-m4 @ c4) ** 2)
            + 0.5 * self.Kt * (tau1**2 + tau2**2 + tau3**2 + tau4**2)
            + 0.5 * self.Ks * (np.arcsin(singa) ** 2 + np.arcsin(singb) ** 2)
            + 0.5 * self.Kc * (np.arcsin(singc) ** 2)
        )
    ...
    def Wn(self, F, G=G0):
        ...
        Wn1 = 0.5 * self.Kn * (-m1 @ c1) ** 2
        Wn2 = 0.5 * self.Kn * (-m2 @ c2) ** 2
        Wn3 = 0.5 * self.Kn * (-m3 @ c3) ** 2
        Wn4 = 0.5 * self.Kn * (-m4 @ c4) ** 2
    ...
    def Wg(self, F, G=G0):
        ...
        Wg1 = 0.5 * self.Kg * kg1**2
        Wg2 = 0.5 * self.Kg * kg2**2
        Wg3 = 0.5 * self.Kg * kg3**2
        Wg4 = 0.5 * self.Kg * kg4**2
    ...
    def Wt(self, F, G=G0):
        ...
        Wt1 = 0.5 * self.Kt * tau1**2
        Wt2 = 0.5 * self.Kt * tau2**2
        Wt3 = 0.5 * self.Kt * tau3**2
        Wt4 = 0.5 * self.Kt * tau4**2
    ...
    def Ws(self, F, G=G0):
        ...
        Wsa = 0.5 * self.Ks * np.arcsin(singa) ** 2
        Wsb = 0.5 * self.Ks * np.arcsin(singb) ** 2
        return Wsa + Wsb
    ...
    def Wc(self, F, G=G0):
        ...
        Wc = 0.5 * self.Kc * np.arcsin(na @ nb) ** 2
