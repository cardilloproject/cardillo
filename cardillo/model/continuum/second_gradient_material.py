...
class Pantobox_beam_network():
    ...
    def e_F(self, F, G, al):
        ...
        e_F = np.einsum('ij,k->ijk', self.I3, self.D[al]) / rho[al] \
            - np.einsum('i,j,k->ijk', e[al],  e[al], self.D[al]) / rho[al]
    ...
    def m_F(self, F, G, al):
        ...
        m1_F = np.einsum('i,jk->ijk', tanga * m1 - e1, self.ga_F(F, G)) \
            + self.e_F(F, G, 1) / cosga - tanga * self.e_F(F, G, 0)

        m2_F = np.einsum('i,jk->ijk', tanga * m2 + e2, self.ga_F(F, G)) \
            - self.e_F(F, G, 0) / cosga + tanga * self.e_F(F, G, 1)
...
def verify_derivatives():
    ...
    P1 = mat.m_F_F(F, G, 0)
    P1_num = mat.v_F_num(F, G, v=mat.m_F)
