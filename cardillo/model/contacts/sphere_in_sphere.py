import numpy as np
from cardillo.math.algebra import cross3, norm3, e3, ax2skew
...
class Sphere_in_sphere():
    ...
    def assembler_callback(self):
        ...
        self.n = lambda t, q: - ( self.subsystem.r_OP(t, q, frame_ID=self.frame_ID, K_r_SP=self.K_r_SP) - self.r_OQ(t) ) / norm3(self.subsystem.r_OP(t, q, frame_ID=self.frame_ID, K_r_SP=self.K_r_SP) - self.r_OQ(t))
    ...
    def g_N(self, t, q):
        return np.array([self.R - self.r - norm3(self.r_OP(t, q) - self.r_OQ(t))])
    ...
    def Wla_N_q(self, t, q, la_N, coo):
        ...
        dense = la_N[0] * (np.einsum('i,ijk->jk', self.n(t, q), self.J_P_q(t, q)) + np.einsum('ik,ij->jk', n_q, self.J_P(t, q)) )
