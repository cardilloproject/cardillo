...
# Constraint on displacement gradient component on boundary surfaces
...
class gradient_constraint():
        ...
        for i in range(self.mesh.nqp):
            ...
            for a in range(self.mesh.nn_el):
                for a_tilde in range(self.la_mesh.nn_el):
                    ...
                    ge_q[a_tilde, self.srf_mesh.nodal_DOF[a, self.x]
                         ] += N_la_eli[a_tilde] * N_X_eli[a, self.x] * w_J0
        return ge_q

    def g_q_dense(self, t, q):
        g_q = np.zeros((self.nla_g, self.nz_srf))
    ...
    def Wla_g_q_el(self, t, qel, lael, el):
        ...
        for i in range(self.mesh.nqp):
            ...
            dF_dq = np.zeros(
                (self.subsystem.dim, self.subsystem.dim, self.mesh.nq_n * self.mesh.nn_el))
            for a in range(self.mesh.nn_el):
                # TODO: reference
                # qa = qel[self.mesh.nodalDOF[a]]
                # F_qp += np.outer(qa, self.subsystem.N_X[el, i, a])
                dF_dq[:, :, self.mesh.nodalDOF[a]] += np.einsum(
                    'ik,j->ijk', np.eye(self.subsystem.dim), self.subsystem.N_X[el, i, a])

            F_qp = self.subsystem.F[el, i]
            F_inv = inverse(F_qp)
            detF = determinant(F_qp)
            dJFinvT_dqe = detF * np.einsum('klmn, mnj->klj', (np.einsum(
                'nm,lk->klmn', F_inv, F_inv) - np.einsum('lm,nk->klmn', F_inv, F_inv)),  dF_dq)
            for a in range(self.mesh.nn_el):
                for a_tilde in range(self.la_mesh.nn_el):
                    la_a_tilde = lael[a_tilde]
                    Wla_g_q_el[self.mesh.nodalDOF[a], :] += la_a_tilde * self.la_mesh.N[el, i, a_tilde] * np.einsum(
                        'klj,l->kj', dJFinvT_dqe,  self.subsystem.N_X[el, i, a]) * self.subsystem.w_J0[el, i]
    ...
    def Wla_g_q(self, t, q, la_g, coo):
        ...
        for el in range(self.mesh.nel):
            ...
            lael = la_g[la_elDOF]
            Wla_g_q[np.ix_(self.mesh.elDOF[el], self.mesh.elDOF[el])
