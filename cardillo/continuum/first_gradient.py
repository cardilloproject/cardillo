...
class FirstGradient:
    def __init__(self, density, material, mesh, Z, z0=None, v0=None, cDOF=[], b=None):
        ...
        if self.dim == 3:
            self.srf_w_J0 = []
            for i in range(6):
                self.srf_w_J0.append(
                    self.mesh.surface_mesh[i].reference_mappings(
                        Z[self.mesh.surface_qDOF[i].ravel()]
                    )
                )
    ...
    def f_pot_q_el(self, ze, el):
        ...
        for i in range(self.nqp):
            ...
            P_F = np.einsum("ik,lj->ijkl", I3, S) + np.einsum(
                "in,njkl->ijkl", F_eli, S_F
            )
