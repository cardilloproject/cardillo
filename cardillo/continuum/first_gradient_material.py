...
class Ogden1997_compressible:
    ...
    def W(self, F):
        ...
        return self.mu1 / 2 * (I1 - 3) - self.mu1 * lnJ + self.mu2 * (J - 1) ** 2
    ...
    def S(self, F):
        ...
        for i in range(len(La)):
            Si = self.mu1 * (1 - 1 / La[i]) + self.mu2 * J * (J - 1) / La[i]
    ...
    def S_F(self, F):
        ...
        for i in range(len(La)):
            Si[i] = self.mu1 * (1 - 1 / La[i]) + self.mu2 * J * (J - 1) / La[i]
        ...
        for i in range(len(La)):
            for j in range(len(La)):
                S_C += Si_Laj[i, j] * np.einsum(
                    "i,j,k,l->ijkl", u[:, i], u[:, i], u[:, j], u[:, j]
                )
                if i != j:
                    ...
                    if isclose(La[i], La[j]):
                        S_C += (
                            0.5
                            * (Si_Laj[j, j] - Si_Laj[i, j])
                            * (
                                np.einsum(
                                    "i,j,k,l->ijkl", u[:, i], u[:, j], u[:, i], u[:, j]
                                )
                                + np.einsum(
                                    "i,j,k,l->ijkl", u[:, i], u[:, j], u[:, j], u[:, i]
                                )
                            )
                        )
                    else:
        ...
        C_F = np.einsum("kj,il->ijkl", F, eye_dim) + np.einsum(
            "ki,jl->ijkl", F, eye_dim
        )
    ...
    def W(self, F):
        ...
        for k in range(len(self.mu)):
            ...
            W += (
                self.mu[k]
                / self.al[k]
                * (sum(wn ** (self.al[k] / 2) for wn in La) + 1 / J ** (self.al[k]) - 3)
            )
        return W
            ...
            for k in range(len(self.mu)):
                ...
                Si += (
                    ...
                    * self.mu[k]
                    / self.al[k]
                    * (
                        self.al[k] / 2 * La[i] ** (self.al[k] / 2 - 1)
                        - self.al[k] / (2 * La[i]) * 1 / J ** (self.al[k])
                    )
                )
            ...
            for k in range(len(self.mu)):
                ...
                Si_Laj[i, i] += (
                    ...
                    * self.mu[k]
                    / 2
                    * (
                        (self.al[k] / 2 - 1) * La[i] ** (self.al[k] / 2 - 2)
                        + (self.al[k] / 2 + 1)
                        * 1
                        / (La[i] ** 2)
                        * 1
        ...
        def W(La):
            ...
            for k in range(len(self.mu)):
                ...
                W += (
                    ...
                    / self.al[k]
                    * (
                        sum(wn ** (self.al[k] / 2) for wn in La)
                        + 1 / J ** (self.al[k])
                        - 3
                    )
                )
