...
class Mesh1D:
    def __init__(
        ...
        if basis == "Lagrange" or basis == "B-spline" or basis == "Lagrange_Disc":
        ...
        if basis == "Lagrange":
            ...
            # total number of nodes
            self.nnodes = self.degree * self.nelement + 1
            self.nnodes_1 = self.nelement - 1 # nnodes_1 = degree * (2 * nelement - 1) + 1

            elDOF_el = np.concatenate(
                [
                    np.arange(self.nnodes_per_element) + i * (self.nnodes + self.nnodes_1) 
                    for i in range(dim_q)]
            )

            for el in range(1, self.nelement + 1):
                if el == 1:
                    self.elDOF[0, :] = elDOF_el.flatten() + (el - 1) * self.degree
                else:
                    self.elDOF[el - 1, :] = self.elDOF[0, :] + (self.nelement - (self.nelement - 1)) * 2 * (el - 1) + (self.degree - 1) * (el - 1)
            self.vtk_cell_type = "VTK_LAGRANGE_CURVE"
        elif basis == "Hermite":
        ...
        if basis == "Lagrange_Disc":
            self.nq = (self.nnodes + self.nnodes_1) * dim_q
            self.nu = (self.nnodes + self.nnodes_1) * dim_u
        else:
        ...
        if basis == "Lagrange_Disc":
            middle_vectors = [np.arange(self.nq)[i:i+dim_q] for i in range(0, self.nq, dim_q)]
            middle_vectors_u = [np.arange(self.nu)[i:i+dim_u] for i in range(0, self.nu, dim_u)]

            def estrai_coppie(degree, nelement):
                nnodes = degree * nelement + 1
                nnodes_1 = self.nelement - 1
                dim = nnodes + nnodes_1
                index_node = np.arange(dim)

                coppie = np.zeros([nnodes_1,2],dtype=int)

                for i in range(nnodes_1):
                    coppie[i,:] = index_node[degree + i * (degree + 1)], index_node[degree + i * (degree + 1) + 1]

                return coppie


            coppie = estrai_coppie(self.degree, self.nelement)
            vv = np.zeros(self.nnodes_1, dtype=int)
            vv_1 = np.zeros(self.nnodes_1, dtype=int)

            for k in range(self.nnodes_1):
                vv[k] = coppie[k,:][0]
                vv_1[k] = coppie[k,:][1]
            ...
            for i, vector in enumerate(middle_vectors):
                if i in vv:
                    combined_vector = np.concatenate([middle_vectors[i], middle_vectors[i+1]])
                    new_vectors.append(combined_vector)
                elif i not in vv_1:
                    new_vectors.append(vector)

            for i, vector_u in enumerate(middle_vectors_u):
                if i in vv:
                    combined_vector_u = np.concatenate([middle_vectors_u[i], middle_vectors_u[i+1]])
                    new_vectors_u.append(combined_vector_u)
                elif i not in vv_1:
                    new_vectors_u.append(vector_u)

            length_max = max(len(vect) for vect in new_vectors)
            length_max_u = max(len(vect_u) for vect_u in new_vectors_u)

            # fill the vector with lower dimension by zeros
            vect_with_zeros = [np.pad(vect, (0, length_max - len(vect)), mode='constant') for vect in new_vectors]
            vect_with_zeros_u = [np.pad(vect_u, (0, length_max_u - len(vect_u)), mode='constant') for vect_u in new_vectors_u]

            # Convert the list in array
            self.nodalDOF = np.vstack(vect_with_zeros)
            self.nodalDOF_u = np.vstack(vect_with_zeros_u) 
            ''' the compatibility matrix has dimension nnodes x 2 * dim_q'''     
        else:
        ...
        '''it remains always the same also for Lagrange_Disc basis'''
    ...
    # The `end_points` method you've provided defines an inner function `select_end_points` and uses it to calculate and return degrees of freedom (DOF) 
    # for specific nodes. Here's a breakdown of what this code does:
    # `select_end_points` Function: This inner function takes keyword arguments using `**kwargs`. 
    # It allows you to specify which nodes you want to consider as "end points" by providing a list of node indices via the `nn_0` keyword argument.
    # `nn_0 = kwargs.get("nn_0", range(self.nnodes))`: This line retrieves the `nn_0` argument from `kwargs`, which is a list of node indices to 
    # be considered as end points. If `nn_0` is not provided, it defaults to all nodes in the range from 0 to `self.nnodes - 1`.
    # `end_points = []`: This initializes an empty list called `end_points` to store the node indices that will be treated as end points.
    # The `for` loop iterates through the node indices in `nn_0` (or the default range if not provided) and appends each index to the `end_points` list.
    # `DOF_x = np.array(end_points)`: This line converts the `end_points` list into a NumPy array called `DOF_x`.
    # `nn_edge = len(end_points)`: This calculates the number of end points, which is simply the length of the `end_points` list.
    # `DOF = np.zeros((self.dim_q, nn_edge), dtype=int)`: This initializes a 2D NumPy array called `DOF` with a shape of `(self.dim_q, nn_edge)` 
    # where `self.dim_q` is the number of degrees of freedom associated with each node.
    # The nested loop (`for i in range(self.dim_q)`) then fills in the `DOF` array with values by adding `i * self.nnodes` to each row of `DOF_x`. 
    # This effectively calculates the DOFs for each degree of freedom and stores them in `DOF`.
    # Finally, the `DOF` array is returned as the result of the `select_end_points` function.
    # Usage in `end_points` Method: The `end_points` method appears to use the `select_end_points` function to calculate DOFs for specific 
    # nodes or end points. The results are then stored and potentially used elsewhere in the code.
    ...
    def end_points(self):
        ...
        ''' be careful, the dimension of the DOF matrix depends on 
            the presence of internal independent degrees'''
