...
class ScalarForceLaw(ABC):
    ...
    def force_q(self, t, q, u):
        return self.la_l(t, self.l(t, q), self.l_dot(t, q, u)) * self.subsystem.l_q(t, q) + self.la_l_dot(t, self.l(t, q), self.l_dot(t, q, u)) * self.subsystem.l_dot_q(t, q)
    
    def force_u(self, t, q, u):
        return self.la_l_dot(t, self.l(t, q), self.l_dot(t, q, u)) * self.subsystem.l_dot_q(t, q, u)

    def h(self, t, q, u):
        return self.force(t, q, u) * self.subsystem.W_l(t, q).reshape(self.subsystem._nu)

    def h_q(self, t, q, u):
        W = self.subsystem.W_l(t, q).reshape(self.subsystem._nu)
        return self.force(t, q, u) * self.subsystem.W_l_q(t, q) + np.outer(W, self.force_q(t, q, u))
