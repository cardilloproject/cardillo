...
def make_R3xSO3_rod(reduced_integration):
    def make(
        L,
        r_OP0,
        A_IK0,
...
def make_R12_rod(polynomial_degree, volume_correction, reduced_integration):
    def make(
        L,
        r_OP0,
        A_IK0,
...
def make_higher_order_R3xSO3_rod(polynomial_degree, reduced_integration):
    def make(
        L,
        r_OP0,
        A_IK0,
        ...
        q0 = HigherOrder_K_R3xSO3_PetrovGalerkin_Quaternion.straight_configuration(
            polynomial_degree,
            polynomial_degree,
            "Lagrange",
            "Lagrange",
        ...
        rod = HigherOrder_K_R3xSO3_PetrovGalerkin_Quaternion(
            cross_section,
            material_model,
            A_rho0,
            K_S_rho0,
...
def make_higher_order_SE3_rod(polynomial_degree, reduced_integration):
    def make(
        L,
        r_OP0,
        A_IK0,
...
# # nnodes_ref = 129
# # nnodes_list = np.array([5, 9, 17, 33, 65], dtype=int)
# # nnodes_ref = 257

# used parameters for Eugster2023
    ...
    for i in range(n_slenderness):
        ...
        def solve(nnodes, rod, volume_correction, reduced_integration):
            ...
            if rod == "SE3":
                ...
                nelements = nnodes - 1
                make_rod = make_R3xSO3_rod(reduced_integration)
            elif rod == "R12p1":
                nelements = nnodes - 1
                ...
                nelements = nnodes - 1
                make_rod = make_higher_order_R3xSO3_rod(
                    1, reduced_integration=reduced_integration
                )
            elif rod == "R3xSO3p2":
                nelements = int((nnodes - 1) / 2)
                make_rod = make_higher_order_R3xSO3_rod(
                    2, reduced_integration=reduced_integration
                )
            elif rod == "SE3p1":
                nelements = nnodes - 1
                make_rod = make_higher_order_SE3_rod(
                    1, reduced_integration=reduced_integration
                )
            elif rod == "SE3p2":
                nelements = int((nnodes - 1) / 2)
        ...
        rod_ref, sol_ref = solve(
            nnodes_ref, reference_rod, volume_correction=False, reduced_integration=True
        )
