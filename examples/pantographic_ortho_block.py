    ...
    if Statics:
        if TractionForce:
            ...
            if "shear" in tests:
                ...
                b = bs
            if "edge" in tests:
                cDOF_xyz = mesh.surface_qDOF[5].ravel()[mesh.surface_mesh[5].edge_qDOF[0]]
                cDOF_x = cDOF_xyz[0]
                cDOF_y = cDOF_xyz[1]
                cDOF_z = cDOF_xyz[2]
                bx = lambda t: Z[cDOF_x] + t * 10
                by = lambda t: Z[cDOF_y]
...
def test_cube():
    ...
    # material parameters
    Lx = 70.0  # Block length in x direction in mm
    Ly = 70.0  # Block length in y direction in mm
    Lz = 70.0  # Block length in x direction in mm
    ...
    mat = Pantobox_beam_network(Ke, Ks, 0.0, 0.0, 0.0, Kc)

    density = 1.0e-3
    tests = ["2x2x10", str(Lx), "edge", "disp_first_grad"]
    ...
    # Weak contstraint
    la_mesh_z = Mesh2D((Xi, Eta), (3, 3), derivative_order=0, nq_n=1)
    ...
    displacement_constraint_shear = Displacement_constraint(continuum, la_mesh_z, srf_id=5, x=0, disp=bs)
    gradient_constraint = Gradient_constraint(continuum, la_mesh_z, srf_id=4, x=0, _X=2)
    ...
    file_path = (
        ...
        / "results"
        / str(f"{file_name}_cube_" + "_".join(tests) + "_nf=" + str(nf))
        / file_name
    )
...
# continuum.post_processing_la(sol.t, la_mesh_y, sol.la_g[:,model.contributions[1].la_gDOF], sol.q[:, continuum.mesh.surface_qDOF[2].ravel()], file_path_la)
