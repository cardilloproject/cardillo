...
if __name__ == "__main__":
    ...
    # beam model
    ############
    beam = TimoshenkoDirectorIntegral(material_model, A_rho0, B_rho0, C_rho0, p_r, p_di, nQP, nEl, q0=Q, Q=Q)

    ##############
    # gravity beam
    ##############
    # # TODO: Why we used this strange gravity?
    # __g = A_rho0 * 9.81 * 5.0e-3 * e3
    if statics:
    ...
    q0 = np.zeros(6)
    q0[2] = -h / 2 + wire_radius # center of mass is shifted (wire starts out of the top cylinder surface)
    u0 = np.zeros(6)
    ...
    ####################
    if statics:
        f_g_bob = Force(lambda t: -t * g * m * e3, bob)
    else:
        f_g_bob = Force(lambda t: -g * m * e3, bob)

    ##############################
    # junction at upper spring end
    ##############################
    A_IK1 = np.vstack((d1[-1], d2[-1], d3[-1])).T # TODO: Is this necessary for the new formulation?
    frame = Frame(r_OP=P[-1], A_IK=A_IK1)
    junction_frame_beam = RigidConnection(frame, beam, frame_ID2=(1,))

    ###############################
    # junction between beam and bob
    ###############################
    junction_beam_bob = RigidConnection(beam, bob, frame_ID1=(0,))
    ...
    if statics:
        ...
        rhs0 = model.h(t0, q0, u0) + model.W_g(t0, q0) @ model.la_g0 + model.W_gamma(t0, q0) @ model.la_gamma0
    ...
    filename = f'Wilberforce_pendulum_p{p}_nEL{nEl}_turns{turns}_t1{t1}_dt{dt}_c{c}'
    ...
    export_path = os.path.join(path, '../blender/' + filename)
