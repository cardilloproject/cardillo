...
def _215_ring(load_type="force", rod_hypothesis_penalty="shear_deformable", VTK_export=False):
    ...
    curve = lambda xi: np.array([R - R * np.cos(xi - start_angle), R * np.sin(xi - start_angle), 0])
    dcurve = lambda xi: np.array([R * np.sin(xi - start_angle), R * np.cos(xi - start_angle), 0])
    ddcurve = lambda xi: np.array([R * np.cos(xi - start_angle), -R * np.sin(xi - start_angle), 0])
    ...
    for i in range(len(t)):
        x_tip_displacement[i] = abs(q[i, cantilever.elDOF_r[element-1][polynomial_degree]]-q0[cantilever.elDOF_r[element-1][polynomial_degree]])
        y_tip_displacement[i] = abs(q[i, cantilever.elDOF_r[element-1][polynomial_degree * 2 + 1]]-q0[cantilever.elDOF_r[element-1][polynomial_degree * 2 + 1]])
