...
def cantilever(
    ...
    A_IK_clamping_1 = lambda t: A_IK_basic(0.0).z()

    clamping_point_ph1 = Frame(A_IK=A_IK_clamping_1)
    clamping_left_c1_ph1 = RigidConnection(
        clamping_point_ph1, cantilever1, frame_ID2=(0,)
    )
    ...
    clamping_left_c1_ph2 = RigidConnection(
        clamping_point_ph2, cantilever1, frame_ID2=(0,)
    )
    ...
    for i in range(len(t_ph1)):
        ...
        vertical_tip_displacement_ph1[i] = q_ph1[
    ...
    for i in range(len(t_ph2)):
        ...
        vertical_tip_displacement_ph2[i] = q_ph2[
            i, cantilever2.qDOF[cantilever2.elDOF_r[nelements - 1, -1]]
        ]

    E_pot_total = np.concatenate((E_pot_total_ph1, E_pot_total_ph2))
    vertical_tip_displacement = np.concatenate(
        (vertical_tip_displacement_ph1, vertical_tip_displacement_ph2)
    )
