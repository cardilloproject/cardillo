...
from cardillo.math import axis_angle2quat, e1, e2, e3, cross3, norm, approx_fprime
...
class RollingCondition:
    ...
    def gamma(self, t, q, u):
        return self.subsystem.v_P(
            t, q, u, B_r_CP=self.subsystem.A_IK(t, q).T @ self.r_SC(t, q)
        )

    def gamma_dot(self, t, q, u, u_dot):
        gamma_q = approx_fprime(q, lambda q: self.gamma(t, q, u))
        gamma_u = gamma_u = self.gamma_u(t, q)
    ...
    def gamma_u(self, t, q):
        return self.subsystem.J_P(
            t, q, B_r_CP=self.subsystem.A_IK(t, q).T @ self.r_SC(t, q)
        )
...
class RollingCondition_g_I_Frame_gamma:
    ...
    def g_ddot(self, t, q, u, u_dot):
        ...
        #     t, q, u, u_dot, B_r_CP=self.subsystem.A_IK(t, q).T @ self.r_SC(t, q)
        # )
        # return a_C @ e3
    ...
    if True:
        ...
        def gamma_dot_q(self, t, q, u, u_dot):
            ...
            approx_fprime(q, lambda q: self.gamma_dot(t, q, u, u_dot)).reshape(
                self.nla_gamma, self.subsystem.nq
            )
