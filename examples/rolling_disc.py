...
def rolling_disc_DMS(rigid_body_case="Euler", constraint_case="velocity_K"):
    ...
    gamma_dot0_pow2 = 4 * (g / r) * sin(beta0) / \
        ((6 - 5 * rho * sin(beta0)) * rho * cos(beta0))
    ...
    K_Omega0 = np.array([
        beta_dot0,
        alpha_dot0 * sin(beta0) + gamma_dot0,
        alpha_dot0 * cos(beta0)
    ])

    # center of mass velocity
    # TODO: Derive these equations!
    v_S0 = np.array([-R * alpha_dot0 + r * alpha_dot0 * sin(beta0),
                     0,
                     0])
    ...
    ax.plot(q_genAlphaFirstOrderVelocity[:, 0], q_genAlphaFirstOrderVelocity[:, 1], q_genAlphaFirstOrderVelocity[:, 2], '-r', label="x-y-z - GenAlphaFirstOrderVeclotiy")
    ...
    if constraint_case == "velocity_K" or constraint_case == "velocity_I":
        ...
        ax.plot(t_genAlphaFirstOrderVelocity[:], la_gamma_genAlphaFirstOrderVelocity[:, 0], '-r', label="la_gamma[0] - GenAlphaFirstOrderVeclotiy")
        ...
        ax.plot(t_genAlphaFirstOrderVelocity[:], la_gamma_genAlphaFirstOrderVelocity[:, 1], '-r', label="la_gamma[1] - GenAlphaFirstOrderVeclotiy")
        ...
        ax.plot(t_genAlphaFirstOrderVelocity[:], la_gamma_genAlphaFirstOrderVelocity[:, 2], '-r', label="la_gamma[2] - GenAlphaFirstOrderVeclotiy")
        ...
        # nonpenetrating contact point
        ax = fig.add_subplot(2, 2, 2)
        ax.plot(t_genAlphaFirstOrderVelocity[:], la_g_genAlphaFirstOrderVelocity[:, 0], '-r', label="la_g - GenAlphaFirstOrderVeclotiy")
    ...
    def create(t, q):
        ...
        bdry, = ax.plot([], [], [], '-k')
        trace, = ax.plot([], [], [], '--k')
        d1_, = ax.plot([x_S, x_S + d1[0]], [y_S, y_S + d1[1]],
                       [z_S, z_S + d1[2]], '-r')
        d2_, = ax.plot([x_S, x_S + d2[0]], [y_S, y_S + d2[1]],
                       [z_S, z_S + d2[2]], '-g')
        d3_, = ax.plot([x_S, x_S + d3[0]], [y_S, y_S + d3[1]],
                       [z_S, z_S + d3[2]], '-b')
    ...
    def update(t, q, COM, bdry, trace, d1_, d2_, d3_):
        ...
        d1_.set_data(np.array([x_S, x_S + d1[0]]),
                     np.array([y_S, y_S + d1[1]]))
        d1_.set_3d_properties(np.array([z_S, z_S + d1[2]]))

        d2_.set_data(np.array([x_S, x_S + d2[0]]),
                     np.array([y_S, y_S + d2[1]]))
        d2_.set_3d_properties(np.array([z_S, z_S + d2[2]]))

        d3_.set_data(np.array([x_S, x_S + d3[0]]),
                     np.array([y_S, y_S + d3[1]]))
