...
class belt_system:
    def __init__(
        ...
        stiffness,
        damping,
        drive_roller_velocity,
        drive_roller_acceleration,
        F_prestress,
        F_friction_smooth,
        F_friction_ns,
        inertia_idleroller_ratio,
        q0,
        u0,
        nelements=10,
    ):
        ...
        self.elDOF = np.vstack(
            (np.arange(self.nelement), np.arange(self.nelement) + 1)
        ).T
    ...
    def f_gyr(self, q, u):
        ...
        for el in range(self.nelement):
            ...
            Delta_v = ue[1] - ue[0]
            f_gyr[elDOF] -= (
                self.density * Delta_s * Delta_v / (self.Delta_x**2) * self.A_e @ ue
            )
        return f_gyr
    ...
    def f_elastic(self, q):
        ...
        for el in range(self.nelement):
            ...
            Delta_s = qe[1] - qe[0]
            f_elastic[elDOF] -= (
                self.stiffness * (self.Delta_x / Delta_s - 1) * np.array([-1, 1])
            )
        return f_elastic
...
if __name__ == "__main__":
    ...
    density = 0.9
    thickness = 2.3e-3
    stiffness = 7.0e7 * thickness
    damping = 1.0e2
    inertia_idleroller_ratio = 1.0e2
    # F_friction = 1e4
    F_friction_smooth = 0
    ...
    solver, label = (
        BackwardEuler(
            ...
            options=SolverOptions(
                reuse_lu_decomposition=True, numerical_jacobian_method="2-point"
            ),
        ),
        "BackwardEuler",
    )
    ...
    ax[1].plot(
        t[::100], (q[::100, -1] - q[0, -1]) - (q[::100, 0] - q[0, 0]), "-k", label=label
    )
    ax[1].grid()
